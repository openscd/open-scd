...
describe('connectedap-104-editor', () => {
  ...
  beforeEach(async () => {
    ...
    connectedAP = document.querySelector(
      'SubNetwork[type="104"] > ConnectedAP'
    )!;

    element = await fixture(
      html`<connectedap-104-editor .element=${connectedAP}>
  ...
  it('has a mwc-fab which deletes the ConnectedAP', () => {
    ...
    expect(editorEvent.args[0][0].detail.action.old.element).to.eql(
      connectedAP
    );
  });
  ...
  it('has a mwc-fab which creates an edit wizard for the ConnectedAP', () => {
    ...
    expect(wizardEvent.args[0][0].detail.wizard()[0].element).to.contain(
      connectedAP
    );
  });
});
