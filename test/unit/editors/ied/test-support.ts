export function getAncestorsFromDO(doElement: Element): Element[] {
  ...
  const lnElement = doElement.closest('SCL')!.querySelector(`LN0[lnType="${lNodeTypeElement!.getAttribute('id')}"], ` +
...
export function getAncestorsFromDA(daElement: Element, type: string): Element[] {
  const doElement = daElement.closest('SCL')!.querySelector(`DO[type="${type}"]`)!;
