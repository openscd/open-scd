...
describe('nsdoc', () => {
  ...
  describe('has an initializeNsdoc function', () => {
    beforeEach(async () => {
      ...
      nsdoc74 = await fetch('/test/testfiles/foundation/testFile74.nsdoc').then(
        response => response.text()
      );
      nsdoc73 = await fetch('/test/testfiles/foundation/testFile73.nsdoc').then(
        response => response.text()
      );
      nsdoc81 = await fetch('/test/testfiles/foundation/testFile81.nsdoc').then(
        response => response.text()
      );
    });
    ...
    it('that has an nsdoc object after loading a correct .nsdoc file into localStorage', async function () {
      ...
      expect(
        nsdocsObject.nsdoc74?.querySelector('NSDoc')?.getAttribute('id')
      ).to.eql('IEC 61850-7-4');
    });
    ...
    describe('has an getDataDescription function', () => {
      ...
      describe('which for LN elements', () => {
        it('returns the title', async function () {
          ...
          expect(nsdocsObject.getDataDescription(ln!).label).to.eql(
            'Some LN title (LLN0)'
          );
        });
      ...
      describe('which for DO elements', () => {
        it('returns the description', async function () {
          const dataObject = validSCL.querySelector(
            'LNodeType[id="Dummy.LLN0"] > DO[name="Beh"]'
          );

          expect(nsdocsObject.getDataDescription(dataObject!).label).to.eql(
            'Some DO description'
          );
        });
        ...
        it('returns the description where the DO is part of a parent class', async function () {
          const dataObject = validSCL.querySelector(
            'LNodeType[id="Dummy.XCBR1"] > DO[name="Beh"]'
          );

          expect(nsdocsObject.getDataDescription(dataObject!).label).to.eql(
            'Some DomainLN Description'
          );
        });

        it('returns the name in case no description can be found', async function () {
          const dataObject = validSCL.querySelector(
            'LNodeType[id="Dummy.LLN0"] > DO[name="Health"]'
          );
      ...
      describe('which for DA elements', () => {
        it('returns the description defined in IEC 61850-7-3', async function () {
          const dataObject = validSCL.querySelector(
            'DOType[id="Dummy.LLN0.Mod"] > DA[name="q"]'
          );
        ...
        it('returns the name in case no description can be found in IEC 61850-7-3', async function () {
          const dataObject = validSCL.querySelector(
            'DOType[id="Dummy.LLN0.Mod"] > DA[name="t"]'
          );
        ...
        it('returns the description defined in IEC 61850-8-1', async function () {
          const dataObject = validSCL.querySelector(
            'DOType[id="Dummy.LLN0.Mod"] > DA[name="SBOw"]'
          );
        ...
        it('which returns the name in case no description can be found in IEC 61850-8-1', async function () {
          const dataObject = validSCL.querySelector(
            'DOType[id="Dummy.LLN0.Mod"] > DA[name="SBO"]'
          );
      ...
      describe('which for BDA elements', () => {
        it('returns the description defined in IEC 61850-7-3', async function () {
          const bdaElement = validSCL.querySelector(
            'DAType[id="AnalogueValue_i"] > BDA[name="i"]'
          );
          const bdaElementParent = validSCL.querySelector(
            'DOType[id="DummySAV"] > DA[name="instMag"]'
          );

          expect(
            nsdocsObject.getDataDescription(bdaElement!, [bdaElementParent!])
              .label
          ).to.eql('Some i description');
        });

        it('returns the name in case no description can be found in IEC 61850-7-3', async function () {
          const bdaElement = validSCL.querySelector(
            'DAType[id="AnalogueValue_i"] > BDA[name="x"]'
          );
          const bdaElementParent = validSCL.querySelector(
            'DOType[id="DummySAV"] > DA[name="instMag"]'
          );
        ...
        it('returns the description defined in IEC 61850-8-1', async function () {
          const bdaElement = validSCL.querySelector(
            'DAType[id="Dummy.LLN0.Mod.SBOw"] > BDA[name="ctlNum"]'
          );
          const bdaElementParent = validSCL.querySelector(
            'DOType[id="Dummy.LLN0.Mod"] > DA[name="SBOw"]'
          );
