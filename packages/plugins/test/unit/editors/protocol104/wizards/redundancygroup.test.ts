...
} from '../../../../../src/editors/protocol104/wizards/redundancygroup.js';
...
describe('Wizards for the Redundancy Group SCL element group', () => {
  ...
  describe('include an edit wizard that', () => {
    beforeEach(async () => {
      const wizard = editRedundancyGroupWizard(
        doc.querySelector(
    ...
    it('does not trigger a editor action to update P elements(s) when not all fields are filled in', async () => {
      input = <WizardTextField>(
        inputs.find(input => input.label === 'TIMEOUT-1')
      );
      input.value = '';
    ...
    it('properly creates a P element if not present', async () => {
      doc
        .querySelector(
          'Communication > SubNetwork[name="W1"] > ConnectedAP[iedName="B1"] > Address > P[type="RG2-TIMEOUT-2"]'
        )
        ?.remove();

      input = <WizardTextField>(
        inputs.find(input => input.label === 'TIMEOUT-2')
      );
      input.value = '77';
      await input.requestUpdate();
    ...
    it('properly deletes a full Redundancy Group', async () => {
      const deleteAction = <HTMLElement>(
        element.wizardUI.dialog?.querySelectorAll('mwc-menu > mwc-list-item')[1]
      );
  ...
  describe('include a create wizard that', () => {
    beforeEach(async () => {
      const wizard = createRedundancyGroupWizard(
        doc.querySelector(
    ...
    it("doesn't trigger a create editor action on primary action without all fields being filled in", async () => {
      const ipElement = <WizardTextField>(
        inputs.find(input => input.label === 'K-FACTOR')
      );
      ipElement.value = '8';
    ...
    it('triggers a create editor action on primary action with all fields being filled in', async () => {
      (<WizardTextField>(
        inputs.find(input => input.label === 'W-FACTOR')
      )).value = '5';
      (<WizardTextField>(
        inputs.find(input => input.label === 'K-FACTOR')
      )).value = '9';
      (<WizardTextField>(
        inputs.find(input => input.label === 'TIMEOUT-0')
      )).value = '15';
      (<WizardTextField>(
        inputs.find(input => input.label === 'TIMEOUT-1')
      )).value = '20';
      ...
      (<WizardTextField>(
        inputs.find(input => input.label === 'TIMEOUT-3')
      )).value = '30';
      await element.requestUpdate();
