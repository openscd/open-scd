...
import { createElement, find, identity, pathParts, } from '../foundation.js';
...
function getLogicalNode(doc, identity) {
...
function hasClientLN(cb, identity) {
    for (const clientLN of Array.from(cb.getElementsByTagName('ClientLN'))) {
        ...
        if (identity.split('>').length === 4) {
            if (iedName === ied?.getAttribute('name') &&
    ...
    return [
        {
          ...
          >${clientLns.map(clientLN => {
                    const ln = (clientLN.getAttribute('prefix') ?? '') +
...
//# sourceMappingURL=clientln.js.map