...
import { cloneElement, createElement, find, getChildElementsByTagName, getValue, identity, isPublic, newLogEvent, newWizardEvent, referencePath, newLnInstGenerator, } from '../foundation.js';
import { patterns } from './foundation/limits.js';
function createLNodeAction(parent) {
    return (inputs, wizard, list) => {
        const selectedLNodeTypes = list.items
            .filter(item => item.selected)
            .map(item => item.value)
            .map(identity => find(parent.ownerDocument, 'LNodeType', identity))
            .filter(item => item !== null);
        const lnInstGenerator = newLnInstGenerator(parent);
        const createActions = selectedLNodeTypes
    ...
    return [
        {
          ...
          >${lNodeTypes.map(lNodeType => {
                    const isDisabled = (lNodeType.getAttribute('lnClass') === 'LLN0' &&
                        getChildElementsByTagName(parent, 'LNode').some(lnode => lnode.getAttribute('lnClass') === 'LLN0')) ||
                        (lNodeType.getAttribute('lnClass') === 'LPHD' &&
                            getChildElementsByTagName(parent, 'LNode').some(lnode => lnode.getAttribute('lnClass') === 'LPHD'));
