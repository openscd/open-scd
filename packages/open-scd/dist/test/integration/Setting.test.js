...
import { newLoadNsdocEvent } from '../../src/addons/Settings.js';
describe('Oscd-Settings', () => {
    let logger;
    let settings;
    beforeEach(async () => {
        localStorage.clear();
        logger = await fixture(html `<mock-setter-logger>
    ...
    it('upload .nsdoc file using event and looks like latest snapshot', async () => {
        ...
        await expect(settings.shadowRoot.querySelector('mwc-dialog[id="settings"]')).to.equalSnapshot();
    });
    it('upload invalid .nsdoc file using event and log event fired', async () => {
        settings.settingsUI.show();
        await settings.settingsUI.updateComplete;
        const nsdocFile = await fetch('/test/testfiles/nsdoc/invalid.nsdoc').then(response => response.text());
        ...
        expect(logger.log[0].title).to.be.equal("Invalid NSDoc (invalid.nsdoc); no 'id' attribute found in file");
    });
    it('upload .nsdoc file with wrong version using event and log event fired', async () => {
        settings.settingsUI.show();
        await settings.settingsUI.updateComplete;
        const nsdocFile = await fetch('/test/testfiles/nsdoc/wrong-version.nsdoc').then(response => response.text());
        logger.dispatchEvent(newLoadNsdocEvent(nsdocFile, 'wrong-version.nsdoc'));
        await logger.requestUpdate();
        await logger.updateComplete;
        expect(logger.log.length).to.be.equal(1);
        expect(logger.log[0].title).to.be.equal('The version of IEC 61850-7-3 NSD (2007B3) does not correlate ' +
