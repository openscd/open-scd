...
describe('ValidateSchema plugin', () => {
    ...
    describe('for valid SCL files', () => {
        ...
        it('zeroissues indication looks like the latest snapshot', async () => await expect(parent.diagnosticUI).to.equalSnapshot());
        it('indicates successful schema validation in the diagnoses pane', async () => {
            const lastEntry = (parent.diagnoses.get('/src/validators/ValidateSchema.js'));
            expect(lastEntry.length).to.equal(1);
            expect(lastEntry[0].title).to.contain('valid2007B XML schema validation successful');
        });
        it('indicates successful schema validation in the log', async () => {
            const lastEntry = parent.log.pop();
            expect(lastEntry.kind).to.equal('info');
            expect(lastEntry.title).to.contain('valid2007B XML schema validation successful');
        });
    });
    describe('for invalid SCL files', () => {
        ...
        it('pushes issues to the diagnostics pane that look like the latest snapshot', async () => await expect(parent.diagnosticUI).to.equalSnapshot());
        it('create issues in diagnose', async () => expect(parent.diagnoses.get('/src/validators/ValidateSchema.js')).to.not
            .be.undefined);
        it('generates error messages in the log', async () => {
            const lastLogEntry = parent.log.pop();
            expect(lastLogEntry.kind).to.equal('warning');
            expect(lastLogEntry.title).to.contain('invalid2007B XML schema validation failed');
        });
    });
});
