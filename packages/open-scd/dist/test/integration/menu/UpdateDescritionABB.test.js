...
describe('Update method for desc attributes in ABB IEDs', () => {
    ...
    it('creates desc attributes for ExtRef elements with existing intAddr attribute', async () => {
        expect(doc.querySelector('IED[name="IED1"] LN[lnClass="CSWI"][inst="1"] ExtRef:not([desc])')).to.exist;
        parent.wizardUI?.dialog
            ?.querySelector('mwc-button[slot="primaryAction"]')
            .click();
        await parent.updateComplete;
        expect(doc.querySelector('IED[name="IED1"] LN[lnClass="CSWI"][inst="1"] ExtRef[desc="GOOSERCV_BIN.3.I1"]')).to.exist;
    });
    it('does not create desc attributes for ExtRef elements with missing intAddr attribute', async () => {
        expect(doc.querySelector('IED[name="IED1"] LN[lnClass="CSWI"][inst="2"] ExtRef:not([desc])')).to.exist;
        parent.wizardUI?.dialog
            ?.querySelector('mwc-button[slot="primaryAction"]')
            .click();
        await parent.updateComplete;
...
//# sourceMappingURL=UpdateDescritionABB.test.js.map