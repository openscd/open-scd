...
describe('NewProject loader', () => {
    ...
    it('creates an empty Edition 2.1 project on wizard primary button click', async () => {
        ...
        expect(parent.doc?.querySelector('Header')?.getAttribute('id')).to.equal(parent.docName.slice(0, -4));
        expect(parent.doc?.querySelector('SCL')?.getAttribute('xmlns')).to.equal('http://www.iec.ch/61850/2003/SCL');
        expect(parent.doc?.querySelector('SCL')?.getAttribute('version')).to.equal('2007');
        expect(parent.doc?.querySelector('SCL')?.getAttribute('revision')).to.equal('B');
        expect(parent.doc?.querySelector('SCL')?.getAttribute('release')).to.equal('4');
    });
