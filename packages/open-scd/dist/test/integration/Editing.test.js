...
describe('Editing-Logging integration', () => {
    ...
    beforeEach(async () => {
        ...
        elm = (await fixture(html `<mock-editor-logger .doc=${doc}></mock-editor-logger>`));
    ...
    describe('has a Create EditorAction type that', () => {
        ...
        it('can be undone', () => {
            elm.dispatchEvent(newActionEvent({ new: { parent, element: newElement } }));
        ...
        it('automatically assignes a missing reference on redo', () => {
            ...
            elm.undo();
            elm.redo();
            expect(parent.querySelector('Bay[name="Q03"]')?.nextElementSibling).to.equal(parent.querySelector('Bay[name="Q01"]'));
        });
        it('properly uses the reference on redo', () => {
            ...
            elm.undo();
            elm.redo();
            expect(parent.querySelector('Bay[name="Q03"]')?.nextSibling).to.equal(parent.lastChild);
        });
    });
    ...
    describe('has a Move EditorAction type that', () => {
        ...
        it('can be undone', () => {
            expect(element.parentElement).to.equal(parent);
            elm.dispatchEvent(newActionEvent({ old: { parent, element }, new: { parent: newParent } }));
    ...
    describe('has a Replace EditorAction type that', () => {
        ...
        it('can be undone', () => {
            elm.dispatchEvent(newActionEvent({ old: { element }, new: { element: newElement } }));
            elm.undo();
            expect(newElement.parentElement).to.be.null;
            expect(element.parentElement).to.equal(parent);
        });
