...
customElements.define('smv-subscribe-later-binding-plugin', SMVSubscribeLaterBindingPlugin);
...
describe('SMV Subscribe Later Binding plugin', () => {
    ...
    it('when selecting an FCDA element with subscriptions it looks like the latest snapshot', async () => {
        ...
        selectFCDAItem(fcdaListElement, 'SMV_Publisher>>CurrentTransformer>currrentOnly', 'SMV_Publisher>>CurrentTransformer>currrentOnlysDataSet>CurrentTransformer/L1 TCTR 1.AmpSv instMag.i (MX)');
        await element.updateComplete;
        await parent.updateComplete;
    ...
    it('when subscribing an available ExtRef then the lists are changed', async () => {
        ...
        selectFCDAItem(fcdaListElement, 'SMV_Publisher>>CurrentTransformer>currentOnly', 'SMV_Publisher>>CurrentTransformer>currentOnlysDataSet>CurrentTransformer/L2 TCTR 1.AmpSv instMag.i (MX)');
        await element.updateComplete;
        await parent.updateComplete;
        await extRefListElement.requestUpdate();
        ...
        expect(extRefListElement['getAvailableExtRefElements']().length).to.be.equal(9);
        ...
        expect(extRefListElement['getAvailableExtRefElements']().length).to.be.equal(8);
    });
    it('when subscribing an available ExtRef then a supervision instance is created', async () => {
        ...
        const fcdaListElement = getFCDABindingList(element);
        const extRefListElement = getExtrefLaterBindingList(element);
        selectFCDAItem(fcdaListElement, 'SMV_Publisher>>CurrentTransformer>fullSmv', 'SMV_Publisher>>CurrentTransformer>fullSmvsDataSet>CurrentTransformer/L2 TCTR 2.AmpSv instMag.i (MX)');
        await element.updateComplete;
        await parent.updateComplete;
        ...
        await element.updateComplete;
        await parent.updateComplete;
        const supervisionInstance = element.doc.querySelector('IED[name="SMV_Subscriber"] LN[lnClass="LSVS"][inst="3"]');
        expect(supervisionInstance).to.exist;
        expect(supervisionInstance?.previousElementSibling?.getAttribute('lnClass')).to.equal('LSVS');
        expect(supervisionInstance?.previousElementSibling?.getAttribute('inst')).to.equal('2');
    });
    it('when unsubscribing a subscribed ExtRef then the lists are changed', async () => {
        const fcdaListElement = getFCDABindingList(element);
        const extRefListElement = getExtrefLaterBindingList(element);
        selectFCDAItem(fcdaListElement, 'SMV_Publisher>>CurrentTransformer>currentOnly', 'SMV_Publisher>>CurrentTransformer>currentOnlysDataSet>CurrentTransformer/L1 TCTR 1.AmpSv q (MX)');
        await element.requestUpdate();
        await extRefListElement.requestUpdate();
        expect(extRefListElement['getSubscribedExtRefElements']().length).to.be.equal(3);
        ...
        expect(extRefListElement['getAvailableExtRefElements']().length).to.be.equal(10);
    });
});
//# sourceMappingURL=SMVSubscriberLaterBinding.test.js.map