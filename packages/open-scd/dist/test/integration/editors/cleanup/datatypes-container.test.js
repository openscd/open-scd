...
describe('cleanup-editor integration: unreferenced control blocks', () => {
    ...
    describe('with a test file loaded', () => {
        ...
        it('correctly removes a LNodeType entry from the SCL', async () => {
            ...
            expect(element.doc.querySelectorAll('LNodeType[id="NotUsedTVTR"]')).to.have.length(0);
        });
        it('correctly removes DOTypes and SDOs from the SCL', async () => {
            ...
            expect(element.doc.querySelectorAll('DOType[id="NotUsedDummy.SPS"], DOType[name="WYE_2_3"], DOType[id="CMV_1"]')).to.have.lengthOf(0);
        });
        it('correctly removes DATypes and those referenced via BDAs from the SCL', async () => {
            ...
            expect(element.doc.querySelectorAll('DAType[id="NotUsedDummy.LPHD1.Sim.Cancel"], DAType[id="OnlySubUsedVector_0"], DAType[id="AnalogValue_0"]')).to.have.lengthOf(0);
        });
        it('correctly removes EnumTypes', async () => {
            ...
            expect(element.doc.querySelectorAll('EnumType[id="NotUsedDir"]')).to.have.lengthOf(0);
        });
        describe('if the Remove subtypes checkbox is unchecked', () => {
            ...
            it('correctly removes two DOTypes and not those referenced via an SDO from the SCL', async () => {
                ...
                expect(element.doc.querySelectorAll('DOType[id="NotUsedDummy.SPS"], DOType[id="WYE_2_3"], DOType[id="Dummy.LLN0.Health.Unused]')).to.have.lengthOf(0);
            ...
            it('correctly removes DATypes and _not_ those referenced via a BDA from the SCL', async () => {
                ...
                expect(element.doc.querySelectorAll('DAType[id="NotUsedDummy.LPHD1.Sim.Cancel"], DAType[id="OnlySubUsedVector_0"]')).to.have.lengthOf(0);
                expect(element.doc.querySelectorAll('DAType[id="AnalogValue_0"]')).to.have.lengthOf(1);
            });
        });
