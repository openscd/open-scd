...
describe('cleanup-editor integration: unreferenced control blocks', () => {
    ...
    describe('with a test file loaded', () => {
        ...
        it('correctly removes all LogControl entries from the SCL', async () => {
            ...
            expect(element.doc.querySelectorAll('LogControl[name="LogNP"]')).to.have.length(0);
        });
        it('correctly removes all GSEControl entries and Address entries from the SCL', async () => {
            ...
            expect(element.doc.querySelectorAll('GSEControl[name="GCB_NP"], GSEControl[name="GCB2_NP"]')).to.have.lengthOf(0);
            // Addresses removed
            expect(element.doc.querySelectorAll('GSE[cbName="GCB_NP"]')).to.have.lengthOf(0);
        ...
        it('correctly removes all SampledValueControl and Address entries from the SCL', async () => {
            ...
            expect(element.doc.querySelectorAll('SampledValueControl')).to.have.lengthOf(1);
            expect(element.doc.querySelectorAll('SampledValueControl[name="MSVCB01_A"]')).to.have.lengthOf(0);
            // Addresses removed
            expect(element.doc.querySelectorAll('SMV[cbName="MSVCB01_A"]')).to.have.lengthOf(0);
        ...
        describe('if the Address checkbox is unchecked', () => {
            ...
            it('correctly removes all SampledValueControl but not Address entries from the SCL', async () => {
                ...
                expect(element.doc.querySelectorAll('SampledValueControl[name="MSVCB01_A"]')).to.have.lengthOf(0);
                // Addresses unchanged
                expect(element.doc.querySelectorAll('SMV[cbName="MSVCB01_A"]')).to.have.lengthOf(1);
            ...
            it('correctly removes all GSEControl entries but not Address entries from the SCL', async () => {
                ...
                // the correct number of GSEControl should remain
                expect(element.doc.querySelectorAll('GSEControl')).to.have.lengthOf(2);
                expect(doc.querySelectorAll('GSEControl[name="GCB_NP"], GSEControl[name="GCB2_NP"]')).to.have.lengthOf(0);
                // Addresses unchanged
                expect(element.doc.querySelectorAll('GSE[cbName="GCB_NP"]')).to.have.lengthOf(1);
        ...
        describe('if the ReportControl filter is enabled', async () => {
            ...
            it('creates 5 delete actions (ReportControl, GSEControl x 2, LogControl, SampledValueControl)', () => {
                const cleanItems = Array.from(element.cleanupList.index.values()).map(index => element.unreferencedControls[index]);
...
//# sourceMappingURL=control-blocks-container.test.js.map