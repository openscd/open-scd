...
describe('cleanup-editor integration: dataset removal', () => {
    ...
    describe('With a test file loaded', () => {
        ...
        it('creates two delete actions', async () => {
            ...
            const cleanItems = Array.from(element.dataSetList.index.values()).map(index => element.unreferencedDataSets[index]);
        ...
        it('correctly removes the datasets from the SCL file', async () => {
            ...
            const remainingDataSetCountCheck = element.doc.querySelectorAll(':root > IED > AccessPoint > Server > LDevice > LN0 > DataSet, :root > IED > AccessPoint > Server > LDevice > LN > DataSet').length === 6;
            // those DataSets selected had best be gone
            const datasetsCorrectlyRemoved = element.doc.querySelectorAll('DataSet[name="GooseDataSet2"], DataSet[name="PhsMeas2"]').length === 0;
            expect(remainingDataSetCountCheck && datasetsCorrectlyRemoved).to.equal(true);
        });
    });
});
//# sourceMappingURL=datasets-container.test.js.map