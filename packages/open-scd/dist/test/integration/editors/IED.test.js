...
describe('IED Plugin', () => {
    ...
    describe('without a doc loaded', () => {
        beforeEach(async () => {
            parent = await fixture(html `<mock-open-scd><ied-plugin></ied-plugin></mock-open-scd>`);
        ...
        describe('containing IEDs', () => {
          ...
          ></mock-open-scd>`);
            ...
            it('when other IED selected then IED Container contains the correct IED', async () => {
                expect(element.shadowRoot?.querySelectorAll('ied-container').length).to.eql(1);
                expect(getIedContainer().shadowRoot?.querySelector('action-pane').shadowRoot
                    ?.innerHTML).to.include('IED1');
                await selectIed('IED3');
                await new Promise(resolve => setTimeout(resolve, 100)); // await animation
                expect(element.shadowRoot?.querySelectorAll('ied-container').length).to.eql(1);
                expect(getIedContainer().shadowRoot?.querySelector('action-pane').shadowRoot
                    ?.innerHTML).to.include('IED3');
            });
            it('when filtering LN Classes then correct number of LN Containers are rendered', async () => {
                expect(getLDeviceContainer(getIedContainer()).shadowRoot.querySelectorAll('ln-container').length).to.eql(5);
                await selectLNClasses('CSWI');
                await new Promise(resolve => setTimeout(resolve, 100)); // await animation
                expect(getLDeviceContainer(getIedContainer()).shadowRoot.querySelectorAll('ln-container').length).to.eql(2);
            });
            it('when other IED selected, all LNs are selected by default', async () => {
                ...
                expect(getIedContainer().shadowRoot?.querySelector('action-pane').shadowRoot
                    ?.innerHTML).to.include('IED3');
                expect(getLDeviceContainer(getIedContainer()).shadowRoot.querySelectorAll('ln-container').length).to.eql(9);
            });
            it('when filtering LNs, if none are selected, all are selected', async () => {
                await selectIed('IED3');
                const oscdFilterButton = (element.shadowRoot.querySelector('oscd-filter-button[id="lnClassesFilter"]'));
                const filterButton = (oscdFilterButton.shadowRoot.querySelector('mwc-icon-button'));
                filterButton.click();
                await element.updateComplete;
                const primaryButton = (oscdFilterButton.shadowRoot.querySelector('mwc-button[slot="primaryAction"]'));
                primaryButton.click();
                await element.updateComplete;
                await new Promise(resolve => setTimeout(resolve, 100)); // await animation
                expect(getLDeviceContainer(getIedContainer()).shadowRoot.querySelectorAll('ln-container').length).to.eql(9);
            ...
            it('then renders the path of elements correctly', async () => {
                ...
                await element.updateComplete;
                expect(getElementPathValue()).to.eql('IED1 / P1 / Server / CircuitBreaker_CB1');
            ...
            ></mock-open-scd>`);
                ...
                it('shows correct wizard when navigating to the DA container that allows for multiple Val and clicking Add', async () => {
                    const lnContainer = getLDeviceContainerByInst(getIedContainer(), 'stage1').shadowRoot.querySelectorAll('ln-container')[1];
                    lnContainer.shadowRoot.querySelector('mwc-icon-button-toggle').on =
                        true;
                    await lnContainer.requestUpdate();
                    // await new Promise(resolve => setTimeout(resolve, 100)); // await animation
                    const doContainer = lnContainer
                    ...
                    // await new Promise(resolve => setTimeout(resolve, 100)); // await animation
                    const daContainer = doContainer.shadowRoot.querySelector('da-container');
                    daContainer
                        .shadowRoot.querySelector('action-pane')
                    ...
                    expect(parent.wizardUI.dialogs[0].querySelectorAll('wizard-textfield')
                        .length).to.equal(3);
                });
            });
            ...
            function getLDeviceContainerByInst(iedContainer, instName) {
                return Array.from(iedContainer
                    .shadowRoot.querySelector('access-point-container')
                    .shadowRoot.querySelector('server-container')
                    .shadowRoot.querySelectorAll('ldevice-container')).find(lDevice => lDevice.element.getAttribute('inst') === instName);
            }
            ...
            async function selectLNClasses(lnClass) {
                const oscdFilterButton = (element.shadowRoot.querySelector('oscd-filter-button[id="lnClassesFilter"]'));
                const filterButton = (oscdFilterButton.shadowRoot.querySelector('mwc-icon-button'));
                filterButton.click();
                await element.updateComplete;
    ...
    async function selectIed(name) {
        const oscdFilterButton = element.shadowRoot.querySelector('oscd-filter-button[id="iedFilter"]');
        const filterButton = (oscdFilterButton.shadowRoot.querySelector('mwc-icon-button'));
        filterButton.click();
        await element.updateComplete;
        const selectItem = (oscdFilterButton.querySelector(`mwc-radio-list-item[value="${name}"]`));
        selectItem.click();
        const primaryButton = (oscdFilterButton.shadowRoot.querySelector('mwc-button[slot="primaryAction"]'));
        primaryButton.click();
        await element.updateComplete;
...
//# sourceMappingURL=IED.test.js.map