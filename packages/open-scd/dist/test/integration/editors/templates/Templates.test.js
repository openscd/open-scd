...
describe('Templates Plugin', () => {
    ...
    let parent;
    beforeEach(async () => {
        parent = await fixture(html `<mock-open-scd><templates-plugin></templates-plugin></mock-open-scd>`);
    ...
    describe('with a doc loaded', () => {
        ...
        describe('having a LNodeType element list that', () => {
            beforeEach(async () => {
                ...
                (element?.shadowRoot
                    ...
                    .querySelector('mwc-list-item')).click();
                await parent.requestUpdate();
            });
            it('opens a LNodeType edit wizard on list element click', () => expect(parent.wizardUI.dialog?.querySelector('wizard-textfield[label="lnClass"]')).to.exist);
            it('allows to reopen the LNodeType edit wizard for the same element', async () => {
                parent.dispatchEvent(newWizardEvent());
                await parent.requestUpdate();
                (element?.shadowRoot?.querySelector('filtered-list:nth-of-type(1) > mwc-list-item')).click();
                await parent.requestUpdate();
                expect(parent.wizardUI.dialog?.querySelector('wizard-textfield[label="lnClass"]')).to.exist;
            });
        });
        describe('having a DOType element list that', () => {
            ...
            it('opens a DOType edit wizard on list element click', () => expect(parent.wizardUI.dialog?.querySelector('wizard-textfield[label="CDC"]')).to.exist);
            it('allows to reopen the DOType edit wizard for the same element', async () => {
                ...
                (element?.shadowRoot
                    ...
                    .querySelector('mwc-list-item')).click();
                await parent.requestUpdate();
                expect(parent.wizardUI.dialog?.querySelector('wizard-textfield[label="CDC"]')).to.exist;
            });
        });
        describe('having a DAType element list that', () => {
            ...
            it('opens a DAType edit wizard on list element click', () => expect(parent.wizardUI.dialog).to.exist);
        ...
        describe('having a EnumType element list that', () => {
            ...
            it('opens a EnumType edit wizard on list element click', () => expect(parent.wizardUI.dialog).to.exist);
    ...
    describe('with a doc loaded having a datatypetemplates section', () => {
        ...
        it('opens an add enumtype wizard', async () => {
            ...
            (parent
                ?.querySelector('templates-plugin')
                ?.shadowRoot?.querySelectorAll('mwc-icon-button[icon="playlist_add"]')[2]).click();
            await parent.updateComplete;
        ...
        it('adding an EnumType with the enumtype wizard', async () => {
            ...
            (parent
                ?.querySelector('templates-plugin')
                ?.shadowRoot?.querySelector('section:last-child mwc-icon-button[icon="playlist_add"]')).click();
            await parent.updateComplete;
            await new Promise(resolve => setTimeout(resolve, 100)); // await animation
