...
describe('DAType wizards', () => {
    ...
    describe('defines a createDATypeWizard', () => {
        ...
        let primayAction;
        beforeEach(async () => {
            const button = (templates?.shadowRoot?.querySelectorAll('mwc-icon-button[icon="playlist_add"]')[2]);
            button.click();
        ...
        it('recursively add missing! subsequent DAType elements', async () => {
            ...
            expect(doc.querySelector('DAType[id="OpenSCD_AnalogueValue_FLOAT32"]')).to
                .exist;
            expect(doc.querySelectorAll('DAType[id="OpenSCD_AnalogueValue_FLOAT32"]')
                .length).to.equal(1);
        });
    });
    describe('defines a dATypeWizard', () => {
        ...
        it('does not edit DAType element without changes', async () => {
            const originData = (doc.querySelector('DAType[id="Dummy.LLN0.Mod.SBOw"]')).cloneNode(true);
            primayAction.click();
            await parent.requestUpdate();
            expect(originData.isEqualNode(doc.querySelector('DAType[id="Dummy.LLN0.Mod.SBOw"]'))).to.be.true;
        });
    });
});
//# sourceMappingURL=datype-wizarding.test.js.map