...
describe('ImportIedsPlugin', () => {
    ...
    describe('imports valid ied elements to empty projects', () => {
        ...
        it('adds the connectedap of the imported ied', async () => {
            element.prepareImport(importDoc, 'valid.iid');
            await parent.updateComplete;
            expect(element.doc.querySelector('SubNetwork[name="NewSubNetwork"] > ConnectedAP[iedName="TestImportIED"]')).to.exist;
        });
        ...
        it('renames TEMPLATE IED element if manufacturer/type has illegal characters', async () => {
            ...
            expect(element.doc?.querySelector(':root > IED[name="FancyVendy_ZMegaY_001"]')).to.exist;
        });
        it('allows multiple import of TEMPLATE IEDs', async () => {
            const templateIED1 = await fetch('/test/testfiles/importieds/template.icd')
                .then(response => response.text())
                .then(str => new DOMParser().parseFromString(str, 'application/xml'));
            element.prepareImport(templateIED1, 'template.icd');
            const templateIED2 = await fetch('/test/testfiles/importieds/template.icd')
                .then(response => response.text())
                .then(str => new DOMParser().parseFromString(str, 'application/xml'));
            await element.updateComplete;
        ...
        it('imports the ConnectedAPs for a TEMPLATE IED', async () => {
            ...
            element.prepareImport(templateIED1, 'template.icd');
            await parent.updateComplete;
            expect(element.doc?.querySelectorAll(':root > Communication >  SubNetwork > ConnectedAP[iedName="FancyVendy_ZMegaY_001"]')).to.exist;
        });
        it('loads unique lnodetypes to the project', async () => {
            expect(element.doc?.querySelectorAll(':root > DataTypeTemplates >  LNodeType')
                .length).to.equal(0);
            element.prepareImport(importDoc, 'template.icd');
            await parent.updateComplete;
            expect(element.doc?.querySelectorAll(':root > DataTypeTemplates >  LNodeType')
    ...
    describe('imports valid ied elements', () => {
        ...
        it('loads unique dotypes to the project', async () => {
            expect(element.doc?.querySelectorAll(':root > DataTypeTemplates >  DOType')
                .length).to.equal(16);
            element.prepareImport(importDoc, 'valid.iid');
            await parent.updateComplete;
            expect(element.doc?.querySelectorAll(':root > DataTypeTemplates >  DOType')
        ...
        it('loads unique datypes to the project', async () => {
            expect(element.doc?.querySelectorAll(':root > DataTypeTemplates >  DAType')
                .length).to.equal(7);
            element.prepareImport(importDoc, 'valid.iid');
            await parent.updateComplete;
            expect(element.doc?.querySelectorAll(':root > DataTypeTemplates >  DAType')
        ...
        it('loads unique enumtypes to the project', async () => {
            expect(element.doc?.querySelectorAll(':root > DataTypeTemplates >  EnumType')
                .length).to.equal(4);
            element.prepareImport(importDoc, 'valid.iid');
            await parent.updateComplete;
            expect(element.doc?.querySelectorAll(':root > DataTypeTemplates >  EnumType')
        ...
        it('adds the connectedap of the imported ied', async () => {
            ...
            expect(element.doc.querySelector('ConnectedAP[iedName="TestImportIED"]'))
                .to.exist;
            expect(element.doc.querySelector('ConnectedAP[iedName="TestImportIED"]')
                ?.parentElement).to.equal(element.doc.querySelector('SubNetwork[name="NewSubNetwork"]'));
        });
        it('creates new subnetwork if not present in the doc', async () => {
            expect(element.doc.querySelector('SubNetwork[name="NewSubNetwork"]')).to
                .not.exist;
...
<eTest1:NewThing>
    <P type="solution"/>
        ...
        it('renders wizard for files containing more than one IED', async () => {
            const multipleIedDoc = await fetch('/test/testfiles/importieds/multipleied.scd')
                .then(response => response.text())
                .then(str => new DOMParser().parseFromString(str, 'application/xml'));
            ...
            expect(element.dialog?.querySelectorAll('mwc-check-list-item').length).to.equal(3);
        });
        it('imports selected IEDs from Import IED wizard', async () => {
            ...
            (element.dialog?.querySelector('mwc-button[slot="primaryAction"]')).click();
            await parent.updateComplete;
            await element.requestUpdate();
            await new Promise(resolve => setTimeout(resolve, 100)); // await complex action
            expect(element.doc.querySelectorAll('IED[name="IED3"]').length).to.equal(1);
    ...
    describe('importing invalid ieds', () => {
      ...
      ></mock-open-scd>`);
        ...
        it('throws duplicate ied name error', async () => {
            ...
            expect(parent.log[0].kind).to.equal('error');
            expect(parent.log[0].title).to.equal('IED element IED2 already in the file');
        });
...
//# sourceMappingURL=ImportIedsPlugin.test.js.map