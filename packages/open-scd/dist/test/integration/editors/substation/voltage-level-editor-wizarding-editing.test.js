...
describe('voltage-level-editor wizarding editing integration', () => {
    describe('edit wizard', () => {
        ...
        beforeEach(async () => {
          ...
          ></mock-wizard-editor>`));
            ...
            nomFreqField = (parent.wizardUI.dialog?.querySelector('wizard-textfield[label="nomFreq"]'));
            numPhasesField = (parent.wizardUI.dialog?.querySelector('wizard-textfield[label="numPhases"]'));
            voltageField = (parent.wizardUI.dialog?.querySelector('wizard-textfield[label="Voltage"]'));
        ...
        it('does not change name attribute if not unique within parent element', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('VoltageLevel')?.getAttribute('name')).to.equal(oldName);
        });
        it('changes name attribute on primary action', async () => {
            nameField.value = 'newName';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('VoltageLevel')?.getAttribute('name')).to.equal('newName');
        });
        it('changes desc attribute on primary action', async () => {
            descField.value = 'newDesc';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('VoltageLevel')?.getAttribute('desc')).to.equal('newDesc');
        });
        ...
        it('changes nomFreq attribute on primary action', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('VoltageLevel')?.getAttribute('nomFreq')).to.equal('30');
        });
        ...
        it('changes numPhases attribute on primary action', async () => {
            ...
            await parent.updateComplete;
            primaryAction.click();
            expect(doc.querySelector('VoltageLevel')?.getAttribute('numPhases')).to.equal('3');
        });
        ...
        it('changes Voltage multiplier on primary action', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('Voltage')?.getAttribute('multiplier')).to.equal('M');
    ...
    describe('open add bay wizard', () => {
        ...
        beforeEach(async () => {
          ...
          ></mock-wizard-editor>`));
            element = parent.querySelector('voltage-level-editor');
            (element?.shadowRoot?.querySelector('mwc-list-item[value="Bay"]')).click();
            await parent.updateComplete;
        ...
        it('does not add bay if name attribute is not unique', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelectorAll('VoltageLevel[name="E1"] > Bay').length).to.equal(2);
        });
        it('does add bay if name attribute is unique', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('VoltageLevel[name="E1"] > Bay[name="SecondBay"]')).to.exist;
        });
    });
    ...
    describe('move action', () => {
        ...
        beforeEach(async () => {
            doc = await fetch('/test/testfiles/valid2007B4.scd')
                ...
                .then(str => new DOMParser().parseFromString(str, 'application/xml'));
            parent = (await fixture(html `<mock-wizard-editor
            >${Array.from(doc?.querySelectorAll('VoltageLevel') ?? []).map(vLevel => html `<voltage-level-editor
                  .element=${vLevel}
                ></voltage-level-editor>`)}
            ></mock-wizard-editor
          >`));
        ...
        it('moves VoltageLevel within Substation', async () => {
            expect(doc.querySelector('VoltageLevel')?.getAttribute('name')).to.equal('E1');
            (element2?.shadowRoot?.querySelector('mwc-icon-button[icon="forward"]')).click();
            await parent.updateComplete;
            element.click();
            await parent.updateComplete;
            expect(doc.querySelector('VoltageLevel')?.getAttribute('name')).to.equal('J1');
        });
    });
    ...
    describe('clone action', () => {
        ...
        beforeEach(async () => {
          ...
          ></mock-wizard-editor>`));
            element = parent.querySelector('voltage-level-editor');
            await parent.updateComplete;
            copyContentButton = (element?.shadowRoot?.querySelector('mwc-icon-button[icon="content_copy"]'));
        });
        ...
        it('removes all LNode elements in the copy', async () => {
            expect(doc.querySelector('VoltageLevel[name="E1"]')?.querySelector('LNode')).to.exist;
            copyContentButton.click();
            await parent.updateComplete;
            expect(doc.querySelector('VoltageLevel[name="E2"]')?.querySelector('LNode')).to.not.exist;
        });
        it('removes all Terminal elements except the grounding in the copy', async () => {
            expect(doc
                .querySelector('VoltageLevel[name="E1"]')
                ?.querySelector('Terminal:not([cNodeName="grounded"])')).to.exist;
            copyContentButton.click();
            await parent.updateComplete;
            expect(doc
                .querySelector('VoltageLevel[name="E2"]')
                ?.querySelector('Terminal:not([cNodeName="grounded"])')).to.not.exist;
        });
        it('removes all ConnectivityNode elements in the copy', async () => {
            expect(doc
                .querySelector('VoltageLevel[name="E1"]')
                ?.querySelector('ConnectivityNode')).to.exist;
            copyContentButton.click();
            await parent.updateComplete;
            expect(doc
                .querySelector('VoltageLevel[name="E2"]')
                ?.querySelector('ConnectivityNode')).to.not.exist;
        });
        it('keeps all Bay elements in the copy', async () => {
            copyContentButton.click();
            await parent.updateComplete;
            expect(doc.querySelector('VoltageLevel[name="E1"]')?.querySelectorAll('Bay')
                .length).to.equal(doc.querySelector('VoltageLevel[name="E2"]')?.querySelectorAll('Bay')
                .length);
        });
        it('keeps all ConductingEquipment elements in the copy', async () => {
            ...
            expect(doc
                .querySelector('VoltageLevel[name="E1"]')
                ?.querySelectorAll('ConductingEquipment').length).to.equal(doc
                .querySelector('VoltageLevel[name="E2"]')
    ...
    describe('open create wizard for element Function', () => {
        ...
        it('does not add Function if name attribute is not unique', async () => {
            expect(doc.querySelector('VoltageLevel[name="E1"] > Function[name="voltLvName"]')).to.exist;
            nameField.value = 'voltLvName';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelectorAll('VoltageLevel[name="E1"] > Function[name="voltLvName"]').length).to.equal(1);
        });
        it('does add Function if name attribute is unique', async () => {
            expect(doc.querySelector('VoltageLevel[name="E1"] > Function[name="someNewFunction"]')).to.not.exist;
            nameField.value = 'someNewFunction';
            await parent.updateComplete;
            primaryAction.click();
            expect(doc.querySelector('VoltageLevel[name="E1"] > Function[name="someNewFunction"]')).to.exist;
        });
    });
});
//# sourceMappingURL=voltage-level-editor-wizarding-editing.test.js.map