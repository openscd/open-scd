...
describe('function-editor wizarding editing integration', () => {
    ...
    describe('open create wizard for element SubFunction', () => {
        ...
        it('does not add SubFunction if name attribute is not unique', async () => {
            expect(doc.querySelector('Substation > Function > SubFunction[name="mySubFunc"]')).to.exist;
            nameField.value = 'mySubFunc';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelectorAll('Substation > Function > SubFunction[name="mySubFunc"]').length).to.equal(1);
        });
        it('does add SubFunction if name attribute is unique', async () => {
            expect(doc.querySelector('Substation > Function > SubFunction[name="someNewFunction"]')).to.not.exist;
            nameField.value = 'someNewFunction';
            await parent.updateComplete;
            primaryAction.click();
            expect(doc.querySelector('Substation > Function > SubFunction[name="someNewFunction"]')).to.exist;
        });
    });
    describe('open edit wizard', () => {
        ...
        it('does not update Function if name attribute is not unique', async () => {
            expect(doc.querySelectorAll('Bay[name="COUPLING_BAY"] > Function[name="bay2Func"]')).to.lengthOf(1);
        ...
        it('does update Function if name attribute is unique', async () => {
            ...
            expect(doc.querySelector('Bay[name="COUPLING_BAY"] > Function[name="bayName"]')).to.not.exist;
        });
    });
    describe('open create wizard for element LNode', () => {
        ...
        it('add selected LNode instances to Function parent', async () => {
            ...
            expect(doc.querySelector('Function > LNode[iedName="None"][lnClass="CSWI"][lnInst="1"]')).to.exist;
            expect(doc.querySelector('Function > LNode[iedName="None"][lnClass="CSWI"][lnInst="2"]')).to.exist;
        });
    });
    describe('has a delete icon button that', () => {
        ...
        it('removes the attached Function element from the document', async () => {
            expect(doc.querySelector('Substation[name="AA1"] > Function[name="myFunc"]')).to.exist;
            await deleteButton.click();
            expect(doc.querySelector('Substation[name="AA1"] > Function[name="myFunc"]')).to.not.exist;
        });
    });
...
//# sourceMappingURL=function-editor.test.js.map