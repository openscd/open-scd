...
describe('conducting-equipment-editor wizarding editing integration', () => {
    describe('edit wizard', () => {
        ...
        beforeEach(async () => {
          ...
          ></mock-wizard-editor>`));
            ...
            await (element?.shadowRoot?.querySelector('*[icon="edit"]')).click();
        ...
        it('does not change name attribute if not unique within parent element', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('ConductingEquipment')?.getAttribute('name')).to.equal(oldName);
        });
        it('changes name attribute on primary action', async () => {
            nameField.value = 'newName';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('ConductingEquipment')?.getAttribute('name')).to.equal('newName');
        });
        it('changes desc attribute on primary action', async () => {
            descField.value = 'newDesc';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('ConductingEquipment')?.getAttribute('desc')).to.equal('newDesc');
        });
    ...
    describe('open lnode wizard', () => {
        ...
        beforeEach(async () => {
          ...
          ></mock-wizard-editor>`));
            element = parent.querySelector('conducting-equipment-editor');
            (element?.shadowRoot?.querySelector('*[icon="account_tree"]')).click();
            await parent.updateComplete;
        });
    ...
    describe('move action', () => {
        ...
        beforeEach(async () => {
            doc = await fetch('/test/testfiles/valid2007B4.scd')
                ...
                .then(str => new DOMParser().parseFromString(str, 'application/xml'));
            parent = (await fixture(html `<mock-wizard-editor
            >${Array.from(doc?.querySelectorAll('Bay[name="COUPLING_BAY"] > ConductingEquipment') ?? []).map(condEq => html `<conducting-equipment-editor
                  .element=${condEq}
                ></conducting-equipment-editor>`)}
            ></mock-wizard-editor
          >`));
        ...
        it('moves ConductingEquipment within Bay', async () => {
            expect(doc.querySelector('ConductingEquipment')?.getAttribute('name')).to.equal('QA1');
            (element2?.shadowRoot?.querySelector('*[icon="forward"]')).click();
            await parent.updateComplete;
            element.click();
            await parent.updateComplete;
            expect(doc.querySelector('ConductingEquipment')?.getAttribute('name')).to.equal('QB1');
        });
    });
    describe('remove action', () => {
        ...
        it('removes ConductingEquipment on clicking delete button', async () => {
            ...
            (element?.shadowRoot?.querySelector('*[icon="delete"]')).click();
            await parent.updateComplete;
    ...
    describe('open create wizard for element EqFunction', () => {
        ...
        it('does not add EqFunction if name attribute is not unique', async () => {
            expect(doc.querySelector('ConductingEquipment > EqFunction[name="myEqFuncQA1"]')).to.exist;
            nameField.value = 'myEqFuncQA1';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelectorAll('ConductingEquipment > EqFunction[name="myEqFuncQA1"]').length).to.equal(1);
        });
        it('does add EqFunction if name attribute is unique', async () => {
            expect(doc.querySelector('ConductingEquipment > EqFunction[name="someNewFunction"]')).to.not.exist;
            nameField.value = 'someNewFunction';
            await parent.updateComplete;
            primaryAction.click();
            expect(doc.querySelector('ConductingEquipment > EqFunction[name="someNewFunction"]')).to.exist;
        });
    });
...
//# sourceMappingURL=conducting-equipment-editor-wizarding-editing.test.js.map