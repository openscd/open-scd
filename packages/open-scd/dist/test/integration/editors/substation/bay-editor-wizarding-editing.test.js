...
describe('bay-editor wizarding editing integration', () => {
    describe('edit wizard', () => {
        ...
        it('changes name attribute on primary action', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('Bay')?.getAttribute('name')).to.equal('newName');
        });
        it('changes desc attribute on primary action', async () => {
            descField.value = 'newDesc';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('Bay')?.getAttribute('desc')).to.equal('newDesc');
        });
    ...
    describe('open add conducting equipment wizard', () => {
        ...
        beforeEach(async () => {
          ...
          ></mock-wizard-editor>`));
            element = parent.querySelector('bay-editor');
            (element?.shadowRoot?.querySelector('mwc-list-item[value="ConductingEquipment"]')).click();
            await parent.requestUpdate();
            await parent.updateComplete;
            nameField = (parent.wizardUI.dialog?.querySelector('wizard-textfield[label="name"]'));
        ...
        it('does not add conducting equipment if name attribute is not unique', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelectorAll('VoltageLevel[name="E1"] > Bay[name="COUPLING_BAY"] > ConductingEquipment[name="QA1"]').length).to.equal(1);
        });
        it('does add conducting equipment if name attribute is unique', async () => {
            ...
            await parent.updateComplete;
            primaryAction.click();
            expect(doc.querySelector('VoltageLevel[name="E1"] > Bay[name="COUPLING_BAY"] > ConductingEquipment[name="QA2"]')).to.exist;
        });
    });
    ...
    describe('move action', () => {
        ...
        beforeEach(async () => {
            doc = await fetch('/test/testfiles/valid2007B4.scd')
                ...
                .then(str => new DOMParser().parseFromString(str, 'application/xml'));
            parent = (await fixture(html `<mock-wizard-editor
            >${Array.from(doc?.querySelectorAll('Bay') ?? []).map(bay => html `<bay-editor .element=${bay}></bay-editor>`)}
            ></mock-wizard-editor
          >`));
        ...
        it('moves Bay within VoltageLevel', async () => {
            expect(doc.querySelector('Bay')?.getAttribute('name')).to.equal('COUPLING_BAY');
    ...
    describe('clone action', () => {
        ...
        it('removes all ConnectivityNode elements in the copy', async () => {
            expect(doc.querySelector('Bay[name="Q01"]')?.querySelector('ConnectivityNode')).to.exist;
            copyContentButton.click();
            await parent.updateComplete;
            expect(doc.querySelector('Bay[name="Q03"]')?.querySelector('ConnectivityNode')).to.not.exist;
        });
    ...
    describe('open create wizard for element Function', () => {
        ...
        it('does not add Function if name attribute is not unique', async () => {
            expect(doc.querySelector('Bay[name="COUPLING_BAY"] > Function[name="bayName"]')).to.exist;
            nameField.value = 'bayName';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelectorAll('Bay[name="COUPLING_BAY"] > Function[name="bayName"]').length).to.equal(1);
        });
        it('does add Function if name attribute is unique', async () => {
            expect(doc.querySelector('Bay[name="COUPLING_BAY"] > Function[name="someNewFunction"]')).to.not.exist;
            nameField.value = 'someNewFunction';
            await parent.updateComplete;
            primaryAction.click();
            expect(doc.querySelector('Bay[name="COUPLING_BAY"] > Function[name="someNewFunction"]')).to.exist;
        });
    });
...
//# sourceMappingURL=bay-editor-wizarding-editing.test.js.map