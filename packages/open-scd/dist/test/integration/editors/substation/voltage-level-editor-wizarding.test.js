...
describe('voltage-level-editor wizarding integration', () => {
    ...
    beforeEach(async () => {
        ...
        parent = await fixture(html `<oscd-wizards .host=${document}
      ...
      ></oscd-wizards>`);
        (parent
            ...
            ?.shadowRoot?.querySelector('mwc-icon-button[icon="edit"]')).click();
    ...
    it('should have correct pattern', async () => {
        ...
        expect(parent.wizardUI
            ...
            .getAttribute('pattern')).to.equal(patterns.unsigned);
        expect(parent.wizardUI
    ...
    describe('the first input element', () => {
        ...
        it('edits only for valid inputs', async () => {
            await fc.assert(fc.asyncProperty(regexString(regExp.tName, 1), async (name) => {
    ...
    describe('the second input element', () => {
        ...
        it('edits only for valid inputs', async () => {
            await fc.assert(fc.asyncProperty(regexString(regExp.desc), async (desc) => {
    ...
    describe('the third input element', () => {
        ...
        it('edits only for valid inputs', async () => {
            await fc.assert(fc.asyncProperty(regexString(regExp.unsigned, 1), async (nomFreq) => {
        ...
        it('rejects action for invalid inputs', async () => {
            await fc.assert(fc.asyncProperty(regexString(inverseRegExp.unsigned, 1), async (nomFreq) => {
                parent.wizardUI.inputs[2].value = nomFreq;
                await parent.updateComplete;
                expect(parent.wizardUI.inputs[2].checkValidity()).to.be.false;
            }));
    ...
    describe('the fourth input element', () => {
        ...
        it('edits only for valid inputs', async () => {
            await fc.assert(fc.asyncProperty(fc.integer(1, 255), async (numPhases) => {
        ...
        it('rejects edition for invalid inputs', async () => {
            await fc.assert(fc.asyncProperty(regexString(inverseRegExp.integer, 1), async (nomFreq) => {
    ...
    describe('the fifth input element', () => {
        ...
        it('edits only for valid inputs', async () => {
            await fc.assert(fc.asyncProperty(regexString(regExp.decimal), async (nomFreq) => {
        ...
        it('rejects edition for invalid inputs', async () => {
            await fc.assert(fc.asyncProperty(regexString(inverseRegExp.decimal, 1), async (voltage) => {
...
//# sourceMappingURL=voltage-level-editor-wizarding.test.js.map