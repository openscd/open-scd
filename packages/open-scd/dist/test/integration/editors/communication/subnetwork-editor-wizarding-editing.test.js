...
describe('subnetwork-editor wizarding editing integration', () => {
    describe('edit wizard', () => {
        ...
        beforeEach(async () => {
            ...
            bitRateField = (parent.wizardUI.dialog?.querySelector('wizard-textfield[label="BitRate"]'));
            primaryAction = (parent.wizardUI.dialog?.querySelector('mwc-button[slot="primaryAction"]'));
        });
        it('closes on secondary action', async () => {
            await (parent.wizardUI.dialog?.querySelector('mwc-button[slot="secondaryAction"]')).click();
        ...
        it('does not change name attribute if not unique within parent element', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('SubNetwork')?.getAttribute('name')).to.equal(oldName);
        });
        it('changes name attribute on primary action', async () => {
            nameField.value = 'newSubNetwork';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('SubNetwork')?.getAttribute('name')).to.equal('newSubNetwork');
        });
        it('changes desc attribute on primary action', async () => {
            descField.value = 'newDesc';
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('SubNetwork')?.getAttribute('desc')).to.equal('newDesc');
        });
        ...
        it('changes type attribute on primary action', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('SubNetwork')?.getAttribute('type')).to.equal('newType');
        });
        ...
        it('changes BitRate multiplier on primary action', async () => {
            ...
            primaryAction.click();
            await parent.updateComplete;
            expect(doc.querySelector('BitRate')?.getAttribute('multiplier')).to.equal('M');
            expect(doc.querySelector('BitRate')?.getAttribute('unit')).to.equal('b/s');
        });
    ...
    describe('add ConnectedAP action', () => {
        ...
        it('adds ConnectedAP on primary action', async () => {
            (element?.shadowRoot?.querySelector('mwc-icon-button[icon="playlist_add"]')).click();
            await parent.requestUpdate();
            await parent.updateComplete;
            await element?.updateComplete;
            newConnectedAPItem = (parent.wizardUI.dialog.querySelector('mwc-check-list-item:nth-child(1)'));
            primaryAction = (parent.wizardUI.dialog?.querySelector('mwc-button[slot="primaryAction"]'));
            expect(doc.querySelector(':root > Communication > SubNetwork[name="StationBus"] > ConnectedAP[iedName="IED3"][apName="P2"]')).to.not.exist;
            newConnectedAPItem.click();
            primaryAction.click();
            await parent.requestUpdate();
            await parent.updateComplete;
            expect(doc.querySelector(':root > Communication > SubNetwork[name="StationBus"] > ConnectedAP[iedName="IED3"][apName="P2"]')).to.exist;
        });
        it('add ConnectedAP with GSE and generates correct addresses', async () => {
          ...
          ></mock-wizard-editor>`));
            ...
            await parent.updateComplete;
            await element?.updateComplete;
            expect(doc.querySelector(':root > Communication > SubNetwork[name="StationBus"] > ConnectedAP[iedName="IED4"][apName="P1"]')).to.not.exist;
            ...
            await parent.requestUpdate();
            await parent.updateComplete;
            const connectedAp = doc.querySelector(':root > Communication > SubNetwork[name="StationBus"] > ConnectedAP[iedName="IED4"][apName="P1"]');
...
//# sourceMappingURL=subnetwork-editor-wizarding-editing.test.js.map