...
import { createSampledValueControlWizard, selectSampledValueControlWizard, } from '../../../src/wizards/sampledvaluecontrol.js';
...
describe('Wizards for SCL element SampledValueControl', () => {
    ...
    describe('define a select wizards that ', () => {
        ...
        describe('with the document element as input', () => {
            beforeEach(async () => {
                ...
                sampledValueControlList = (element.wizardUI.dialog?.querySelector('filtered-list'));
                await sampledValueControlList.updateComplete;
            });
            it('shows all SampledValueControl elements within a project', () => expect(sampledValueControlList.items.length).to.equal(doc.querySelectorAll('SampledValueControl').length));
            it('opens edit wizard for selected SampledValueControl element on click', async () => {
                const reportItem = sampledValueControlList.items[0];
                reportItem.click();
                ...
                expect(nameField.value).to.equal(doc.querySelectorAll('SampledValueControl')[0].getAttribute('name'));
            });
            describe('has an add SampledValueControl primary button that', () => {
                ...
                it('opens a potential list of host IEDs for the SampledValueControl element', async () => expect(iEDPicker).to.exist);
            });
        });
        describe('with a specific IED as input', () => {
            beforeEach(async () => {
                const wizard = selectSampledValueControlWizard(doc.querySelector('IED[name="IED3"]'));
                element.dispatchEvent(newWizardEvent(() => wizard));
                await element.requestUpdate();
                sampledValueControlList = (element.wizardUI.dialog?.querySelector('filtered-list'));
                await sampledValueControlList.updateComplete;
            });
            it('shows all SampledValueControl elements within an IED', () => expect(sampledValueControlList.items.length).to.equal(doc.querySelectorAll('IED[name="IED3"] SampledValueControl').length));
            it('opens edit wizard for selected SampledValueControl element on click', async () => {
                const reportItem = sampledValueControlList.items[0];
                reportItem.click();
                await new Promise(resolve => setTimeout(resolve, 20)); // await animation
            ...
            describe('has an add SampledValueControl primary button that', () => {
                ...
                it('opens the create wizard for the SampledValueControl element', async () => expect(nameField).to.exist);
            });
        });
    });
    describe('defines an edit wizard that', () => {
        ...
        describe('loaded without SMV element reference', () => {
            beforeEach(async () => {
                ...
                element.dispatchEvent(newWizardEvent(() => selectSampledValueControlWizard(parentIED)));
                await element.requestUpdate();
                await new Promise(resolve => setTimeout(resolve, 20)); // await animation
                const sampledValueControlBlock = ((element.wizardUI.dialog?.querySelector('filtered-list')).items[0]);
            ...
            it('rejects name attribute starting with decimals', async () => {
                expect(parentIED
                    ...
                    ?.getAttribute('name')).to.not.equal('4adsasd');
            ...
            it('edits name attribute on primary action', async () => {
                expect(parentIED
                    .querySelectorAll('SampledValueControl')[0]
                    ?.getAttribute('name')).to.not.equal('myNewName');
                nameField.value = 'myNewName';
                await element.requestUpdate();
                primaryAction.click();
                expect(parentIED
                    .querySelectorAll('SampledValueControl')[0]
                    ?.getAttribute('name')).to.equal('myNewName');
            });
            ...
            it('does not render SMV edit button', async () => {
                const editSMvButton = (Array.from(element.wizardUI.dialog.querySelectorAll('mwc-menu > mwc-list-item')).find(item => item.innerHTML.includes(`[scl.Communication]`)));
        ...
        describe('loaded with SMV element reference', () => {
            ...
            it('opens a edit wizard for SMV on edit SMV button click', async () => {
                ...
                editSMvButton.click();
                await new Promise(resolve => setTimeout(resolve, 100)); // await animation
                const macField = (element.wizardUI.dialog?.querySelector('wizard-textfield[label="MAC-Address"]'));
                await macField.updateComplete;
                expect(macField.value).to.equal(doc
                    .querySelector('SMV > Address > P[type="MAC-Address"]')
                    ?.textContent?.trim());
            });
            it('opens a edit wizard for SMV on edit SMV button click', async () => {
                const editSmvOptsButton = (Array.from(element.wizardUI.dialog.querySelectorAll('mwc-menu > mwc-list-item')).find(item => item.innerHTML.includes(`[scl.SmvOpts]`)));
                await element.wizardUI.dialog?.requestUpdate();
                expect(editSmvOptsButton).to.exist;
                editSmvOptsButton.click();
                await new Promise(resolve => setTimeout(resolve, 100)); // await animation
                const macField = (element.wizardUI.dialog?.querySelector('wizard-checkbox[label="refreshTime"]'));
                await macField.updateComplete;
            ...
            it('removes the SampledValueControl element and its referenced elements on remove button click', async () => {
                expect(doc.querySelector('IED[name="IED3"] SampledValueControl[name="MSVCB01"]')).to.exist;
                expect(doc.querySelector('IED[name="IED3"] DataSet[name="PhsMeas1"]'))
                    .to.exist;
                expect(doc.querySelector('SMV[cbName="MSVCB01"]')).to.exist;
                const deleteButton = (Array.from(element.wizardUI.dialog.querySelectorAll('mwc-menu > mwc-list-item')).find(item => item.innerHTML.includes(`[remove]`)));
                await element.wizardUI.dialog?.requestUpdate();
                deleteButton.click();
                expect(doc.querySelector('IED[name="IED3"] SampledValueControl[name="MSVCB01"]')).to.not.exist;
                expect(doc.querySelector('IED[name="IED3"] DataSet[name="PhsMeas1"]'))
                    .to.not.exist;
    ...
    describe('defines a create wizards that', () => {
        let primaryAction;
        beforeEach(async () => {
            const wizard = createSampledValueControlWizard(doc.querySelector('IED[name="IED3"] LN0'));
            element.dispatchEvent(newWizardEvent(() => wizard));
            await element.requestUpdate();
        ...
        it('creates a new instance of a SampledValueControl element', () => {
            expect(doc.querySelectorAll('IED[name="IED3"] LN0 > SampledValueControl')).to.have.lengthOf(1);
            primaryAction.click();
            expect(doc.querySelectorAll('IED[name="IED3"] LN0 > SampledValueControl')).to.have.lengthOf(2);
        });
        it('creates a new instance of a DataSet element referenced from SampledValueControl', () => {
            expect(doc.querySelectorAll('IED[name="IED3"] LN0 > DataSet')).to.have.lengthOf(1);
            primaryAction.click();
            expect(doc.querySelectorAll('IED[name="IED3"] LN0 > DataSet')).to.have.lengthOf(2);
        });
        it('creates a new instance of a SMV element', () => {
            expect(doc.querySelectorAll('ConnectedAP[iedName="IED3"][apName="P1"] SMV')).to.have.lengthOf(1);
            primaryAction.click();
            expect(doc.querySelectorAll('ConnectedAP[iedName="IED3"][apName="P1"] SMV')).to.have.lengthOf(2);
        });
    });
});
//# sourceMappingURL=sampledvaluecontrol-wizarding-editing.test.js.map