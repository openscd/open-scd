...
describe('action-pane', () => {
    ...
    beforeEach(async () => {
        element = await fixture(html `<action-pane header="test header" label="${label}"></action-pane>`);
    ...
    it('renders the title on the <h1>', async () => {
        element.level = 1;
        await element.updateComplete;
        expect(element.shadowRoot?.querySelector('h1').getAttribute('title')).to.be.equals(label);
    });
    it('renders the title on the <h2>', async () => {
        element.level = 2;
        await element.updateComplete;
        expect(element.shadowRoot?.querySelector('h2').getAttribute('title')).to.be.equals(label);
    });
    it('renders the title on the <h3>', async () => {
        element.level = 3;
        await element.updateComplete;
        expect(element.shadowRoot?.querySelector('h3').getAttribute('title')).to.be.equals(label);
    });
    it('renders the title on the <h4>', async () => {
        element.level = 4;
        await element.updateComplete;
        expect(element.shadowRoot?.querySelector('h4').getAttribute('title')).to.be.equals(label);
    });
    it('does not set contrasted class property with odd level', async () => {
        ...
        expect(element.shadowRoot
            ...
            ?.classList.contains('contrasted')).to.be.false;
    });
    it('sets contrasted class property with even levels', async () => {
        ...
        expect(element.shadowRoot
            ?.querySelector('section')
            ?.classList.contains('contrasted')).to.be.true;
    });
    ...
    describe('with existing parent action-pane', () => {
        let parent;
        const bay = (new DOMParser().parseFromString(`<Bay name="name" desc="desc"></Bay>`, 'application/xml').documentElement);
        beforeEach(async () => {
            parent = await fixture(html `<action-pane header="parent header"
...
//# sourceMappingURL=action-pane.test.js.map