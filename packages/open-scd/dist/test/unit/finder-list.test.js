...
function getDisplayString(entry, path) {
...
describe('finder-list', () => {
    ...
    it('displays nothing with default properties', () => expect(element).property('container').to.be.empty);
    ...
    describe('given a one-element .path with no header or entries', () => {
        beforeEach(async () => {
            element = await fixture(html `<finder-list .path=${['e5']} .read=${read}></finder-list>`);
            await element.loaded;
        });
        it('displays no columns', async () => expect(element).property('container').property('children').to.be.empty);
    });
    describe('given a .path and a .read method', () => {
        let items;
        beforeEach(async () => {
            element = await fixture(html `<finder-list .path=${pathA} .read=${read}></finder-list>`);
            await element.loaded;
            items = Array.from(element.shadowRoot?.querySelectorAll('mwc-list-item') ?? []);
        });
        it('displays one column per path element', () => expect(element)
            .property('container')
            .property('children')
        ...
        describe('when provided with .getTitle and .getDisplayString methods', () => {
            ...
            it('overrides filtered-list searchFieldLabels using .getTitle', () => {
                expect(element.container.querySelector('filtered-list'))
                    ...
                    .to.satisfy((l) => l.startsWith('Testing '));
            });
            it('overrides list-item text content using .getDisplayString', () => {
                expect(element.container.querySelector('mwc-list-item'))
                    .property('text')
                    .to.satisfy((t) => t.startsWith('Testing '));
            });
            it('looks like its latest snapshot', async () => await expect(element).shadowDom.to.equalSnapshot());
        });
        describe('when an item in the last column is selected', () => {
            ...
            it('appends a new column to the container', () => expect(element)
                .property('container')
                .property('children')
                .to.have.lengthOf(pathA.length + 1));
            it("renders the selected directory's header at the top of the new column", () => expect(element)
                .property('container')
                .property('lastElementChild')
                .property('firstElementChild')
                .to.have.text(directory)
                .and.to.have.property('tagName', 'H2'));
            it("renders the selected directory's children into a list below the header", () => expect(element)
                .property('container')
                .property('lastElementChild')
                .property('lastElementChild')
                .property('children')
        ...
        describe('when an item in the first column is selected', () => {
            ...
            it('replaces all but the first column with a new column', () => expect(element)
                .property('container')
                .property('children')
                .to.have.lengthOf(2));
            it("renders the selected directory's header at the top of the new column", () => expect(element)
        ...
        describe('when the selected item in the first column is deselected', () => {
            ...
            it('renders only the first column', () => expect(element)
                .property('container')
                .property('children')
                .to.have.lengthOf(1));
            it('looks like its latest snapshot', async () => await expect(element).shadowDom.to.equalSnapshot());
    ...
    describe('given the "multi" attribute, some .paths, and a .read method', () => {
        let items;
        beforeEach(async () => {
            element = await fixture(html `<finder-list .paths=${paths} .read=${read} multi></finder-list>`);
            await element.loaded;
            items = Array.from(element.shadowRoot?.querySelectorAll('mwc-list-item') ?? []);
        });
        it('displays one column per element of the longest path', () => expect(element)
            .property('container')
            .property('children')
            .to.have.lengthOf(Math.max(...paths.map(p => p.length))));
        it('displays one header and one list of entries per maximum length path in the last column', () => expect(element)
            .property('container')
            .property('lastElementChild')
            .property('children')
            .to.have.lengthOf(2 *
        ...
        describe('when an item in the first column is selected', () => {
            ...
            it("adds the selected directory's header to the second column", () => expect(element)
                .property('container')
                .descendant('.column:nth-child(2)')
                .descendant('h2:nth-child(3)')
                .to.have.text(directory));
            it("adds the selected directory's entries to the second column", () => expect(element)
                .property('container')
                .descendant('.column:nth-child(2)')
        ...
        describe('when a selected item in the first column is deselected', () => {
            ...
            it('removes the deselected directory from the second column', () => expect(element)
                .property('container')
                .descendant('.column:nth-child(2)')
                .to.not.contain.html(`<h2><!---->${entries[paths[0][0]][1]}<!----></h2>`));
            it('looks like its latest snapshot', async () => await expect(element).shadowDom.to.equalSnapshot());
        });
    });
});
//# sourceMappingURL=finder-list.test.js.map