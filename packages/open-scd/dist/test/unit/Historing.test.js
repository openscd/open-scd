...
describe('HistoringElement', () => {
    let element;
    beforeEach(async () => {
        element = await fixture(html `<mock-logger></mock-logger>`);
    });
    ...
    it('cannot undo warning messages', () => {
        element.dispatchEvent(newLogEvent({ kind: 'warning', title: 'test warning' }));
        expect(element).property('log').to.have.lengthOf(1);
        expect(element).property('canUndo').to.be.false;
    });
    ...
    it('has no previous action', () => expect(element).to.have.property('previousAction', -1));
    it('has no edit count', () => expect(element).to.have.property('editCount', -1));
    it('has no next action', () => expect(element).to.have.property('nextAction', -1));
    it('renders a placeholder message', () => expect(element.logUI).to.contain('mwc-list-item[disabled]'));
    ...
    it('shows a snackbar on an issue', () => {
        ...
        element.dispatchEvent(newIssueEvent({
            validatorId: 'val',
            title: 'test issue',
        }));
    ...
    describe('with an action logged', () => {
        ...
        it('cannot redo', () => expect(element).property('canRedo').to.be.false);
        it('has no previous action', () => expect(element).to.have.property('previousAction', -1));
        it('has an edit count', () => expect(element).to.have.property('editCount', 0));
        it('has no next action', () => expect(element).to.have.property('nextAction', -1));
        it('does not log derived actions', () => {
            ...
            element.dispatchEvent(newLogEvent({
                kind: 'action',
                title: 'test MockAction',
                action: element.history[0].action,
            }));
            expect(element).property('history').to.have.lengthOf(1);
        });
        ...
        it('renders a history message for the action', () => expect(element.historyUI).to.contain.text('test'));
        describe('with a second action logged', () => {
            ...
            it('has a previous action', () => expect(element).to.have.property('previousAction', 0));
            it('has an edit count', () => expect(element).to.have.property('editCount', 1));
            it('has no next action', () => expect(element).to.have.property('nextAction', -1));
            describe('with an action undone', () => {
                beforeEach(() => element.undo());
                it('has no previous action', () => expect(element).to.have.property('previousAction', -1));
                it('has an edit count', () => expect(element).to.have.property('editCount', 0));
                it('has a next action', () => expect(element).to.have.property('nextAction', 1));
                ...
                describe('with the second action undone', () => {
                    beforeEach(() => element.undo());
                    it('cannot undo any funther', () => expect(element.undo()).to.be.false);
                });
                describe('with the action redone', () => {
                    ...
                    it('has an edit count', () => expect(element).to.have.property('editCount', 1));
                    it('has no next action', () => expect(element).to.have.property('nextAction', -1));
                    it('cannot redo any further', () => expect(element.redo()).to.be.false);
                });
            });
        });
    });
    describe('with an issue incoming', () => {
        ...
        it('saves the issue to diagnose', () => {
            expect(element.diagnoses.get('/src/validators/ValidateSchema.js')).to
                .exist;
            const issue = element.diagnoses.get('/src/validators/ValidateSchema.js')[0];
            expect(issue.title).to.equal('test run 1');
        });
        it('does not contain issues from another validator', () => expect(element.diagnoses.has('/src/validators/ValidateTemplates.js')).to
            .be.false);
        describe('with another issue coming in - new validator', () => {
            ...
            it('keeps old issues from the other validator', () => {
                expect(element.diagnoses.get('/src/validators/ValidateSchema.js')).to
                    .exist;
                expect(element.diagnoses.get('/src/validators/ValidateSchema.js').length).to.equal(1);
                const issue = element.diagnoses.get('/src/validators/ValidateSchema.js')[0];
                expect(issue.title).to.equal('test run 1');
            });
            it('in parallel saves the issues of the new validator', () => {
                expect(element.diagnoses.get('/src/validators/ValidateTemplates.js')).to
                    .exist;
                expect(element.diagnoses.get('/src/validators/ValidateTemplates.js').length).to.equal(1);
                const issue = element.diagnoses.get('/src/validators/ValidateTemplates.js')[0];
                expect(issue.title).to.equal('test run 3');
            });
        });
    });
});
//# sourceMappingURL=Historing.test.js.map