...
describe('Plugin that creates with some user input a virtual template IED - SPECIFICATION', () => {
    ...
    `);
        ...
        checkItems = Array.from(element.dialog.querySelectorAll('mwc-check-list-item') ?? []);
        manufacturer = element.dialog.querySelector('wizard-textfield[label="manufacturer"]');
        apName = element.dialog.querySelector('wizard-textfield[label="AccessPoint name"]');
        primaryAction = (element.dialog.querySelector('mwc-button[slot="primaryAction"]'));
    });
    it('looks like the latest snapshot', async () => await expect(element.dialog).dom.to.equalSnapshot());
    it('shows all LNode that is not class LLN0 as check list items', () => expect(checkItems.length).to.equal(doc.querySelectorAll('LNode[iedName="None"]:not([lnClass="LLN0"])').length));
...
//# sourceMappingURL=VirtualTemplateIED.test.js.map