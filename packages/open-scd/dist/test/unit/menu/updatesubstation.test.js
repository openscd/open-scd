...
describe('isValidReference', () => {
    ...
    it('returns true for lNodeIdentity pointing to logical node in the IED', () => {
        const lNode = ours.querySelector('LNode[iedName="IED1"][ldInst="Disconnectors"][prefix="DC"][lnClass="XSWI"][lnInst="1"]');
        expect(isValidReference(ours, identity(lNode))).to.be.true;
    });
    it('checks reference for client side logical nodes', () => {
        const lNode = ours.querySelector('LNode[iedName="IED1"][prefix="DC3"][lnClass="XSWI"][lnInst="2"]');
        expect(isValidReference(ours, identity(lNode))).to.be.true;
    });
    ...
    it('returns false for incorrect lNodeIdentities', () => {
        const lNodeMissingIedNAme = ours.querySelector('LNode[iedName="IED1"][prefix="DC"][lnClass="XSWI"][lnInst="2"]');
        lNodeMissingIedNAme?.removeAttribute('iedName');
        expect(isValidReference(ours, identity(lNodeMissingIedNAme))).to.be.false;
        const lNodeMissingLnClass = ours.querySelector('LNode[iedName="IED1"][prefix="DC"][lnClass="XSWI"][lnInst="2"]');
...
//# sourceMappingURL=updatesubstation.test.js.map