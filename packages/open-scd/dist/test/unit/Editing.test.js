...
describe('EditingElement', () => {
    ...
    beforeEach(async () => {
        ...
        elm = (await fixture(html `<mock-editor .doc=${doc}></mock-editor>`));
    ...
    it('triggers getReference with missing reference on Create Action', () => {
        ...
        expect(parent.querySelector('Bay')?.nextElementSibling).to.equal(parent.querySelector('Bay[name="Q01"]'));
    });
    it('ignores getReference with existing reference on Create Action', () => {
        ...
        expect(parent.querySelector('Bay[name="Q03"]')?.nextElementSibling).to.equal(parent.querySelector('Bay[name="Q02"]'));
    });
    ...
    it('replaces an element on receiving an Replace action', () => {
        ...
        expect(parent.querySelector('newBay')?.nextElementSibling).to.equal(parent.querySelector('Bay[name="Q02"]'));
    });
    it('does not replace an element in case of name conflict', () => {
        ...
        expect(parent.querySelector('Bay[name="Q01"]')?.nextElementSibling).to.equal(parent.querySelector('Bay[name="Q02"]'));
    });
    ...
    it('triggers getReference with missing reference on Move action', () => {
        ...
        expect(elm.doc.querySelector('VoltageLevel[name="J1"] > Bay[name="Q01"]'))
            .to.not.be.null;
        expect(elm.doc.querySelector('VoltageLevel[name="J1"] > Bay[name="Q01"]')
            ?.nextElementSibling).to.equal(elm.doc.querySelector('VoltageLevel[name="J1"] > Function'));
    });
    it('does not move an element in case of name conflict', () => {
        ...
        expect(elm.doc.querySelector('VoltageLevel[name="J1"] > Bay[name="Q01"]'))
            .to.not.be.null;
        expect(elm.doc.querySelector('VoltageLevel[name="J1"] > Bay[name="Q01"]')
            ?.nextElementSibling).to.be.null;
    });
    it('updates an element on receiving an Update action', () => {
        const newAttributes = {};
        newAttributes['name'] = 'Q03';
        elm.dispatchEvent(newActionEvent(createUpdateAction(element, newAttributes)));
    ...
    it('does not update an element in case of id conflict', () => {
        ...
        elm.dispatchEvent(newActionEvent(createUpdateAction(doc.querySelector('LNodeType'), newAttributes)));
...
//# sourceMappingURL=Editing.test.js.map