...
import { diffSclAttributes, diffSclChilds, identityForCompare, isSame, renderDiff, } from '../../../src/foundation/compare.js';
describe('compas-compare-dialog', () => {
    let oldSclElement;
    let newSclElement;
    beforeEach(async () => {
        oldSclElement = await fetch('/test/testfiles/foundation/compare-original.cid')
            .then(response => response.text())
            .then(str => new DOMParser().parseFromString(str, 'application/xml'))
            .then(document => document.documentElement);
        newSclElement = await fetch('/test/testfiles/foundation/compare-changed.cid')
            .then(response => response.text())
            .then(str => new DOMParser().parseFromString(str, 'application/xml'))
            .then(document => document.documentElement);
    });
    describe('identityForCompare', () => {
        it('will return the identity of the sub element, not the full identity', () => {
            const voltageLevel = oldSclElement.querySelector('VoltageLevel[name="S1 30kV"]');
        ...
        it('will return the identity of the main element, meaning the full identity', () => {
            const substation = oldSclElement.querySelector('Substation[name="Substation 1"]');
    ...
    describe('isSame', () => {
        ...
        it('will return true when the same elements from different sources are passed', () => {
            const newVoltageLevel = newSclElement.querySelector('VoltageLevel[name="S1 30kV"]');
            const oldVoltageLevel = oldSclElement.querySelector('VoltageLevel[name="S1 30kV"]');
        ...
        it('will return false when the different elements of the same type are passed', () => {
            const voltageLevel = oldSclElement.querySelector('VoltageLevel[name="S1 30kV"]');
            const differentVoltageLevel = oldSclElement.querySelector('VoltageLevel[name="S1 380kV"]');
    ...
    describe('diffSclAttributes', () => {
        it('no attributes changed', () => {
            const newVoltageLevel = newSclElement.querySelector('VoltageLevel[name="S1 30kV"]');
            const oldVoltageLevel = oldSclElement.querySelector('VoltageLevel[name="S1 30kV"]');
            const diffAttributes = diffSclAttributes(newVoltageLevel, oldVoltageLevel, {}, newVoltageLevel);
            expect(diffAttributes).to.have.length(0);
        });
        it('one attribute has changed', () => {
            const newVoltageLevel = newSclElement.querySelector('VoltageLevel[name="S1 110kV"]');
            const oldVoltageLevel = oldSclElement.querySelector('VoltageLevel[name="S1 110kV"]');
            const diffAttributes = diffSclAttributes(newVoltageLevel, oldVoltageLevel, {}, newVoltageLevel);
        ...
        it('only name changed on copied element', () => {
            const newSubstation = newSclElement.querySelector('Substation[name="Substation 1 (Copy)"]');
            const oldSubstation = oldSclElement.querySelector('Substation[name="Substation 1"]');
    ...
    describe('diffSclChilds', () => {
        it('all children can be updated', () => {
            const newVoltageLevel = newSclElement.querySelector('VoltageLevel[name="S1 380kV"]');
            const oldVoltageLevel = oldSclElement.querySelector('VoltageLevel[name="S1 380kV"]');
            ...
            const updatedChilds = diffChilds.filter(diff => diff.newValue !== null && diff.oldValue !== null);
