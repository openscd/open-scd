...
import { cloneElement, depth, find, findControlBlocks, findFCDAs, getChildElementsByTagName, getNameAttribute, getReference, getSclSchemaVersion, getUniqueElementName, identity, invert, isCreate, isDelete, isMove, isReplace, isSame, isSimple, newActionEvent, newLnInstGenerator, newPendingStateEvent, newWizardEvent, tags, minAvailableLogicalNodeInstance, } from '../../src/foundation.js';
...
describe('foundation', () => {
    ...
    let publicElement;
    beforeEach(async () => {
        scl1 = (await fetch('/test/testfiles/valid2007B4.scd')
            .then(response => response.text())
            .then(str => new DOMParser().parseFromString(str, 'application/xml'))).documentElement;
        scl2 = (await fetch('/test/testfiles/valid2003.scd')
            .then(response => response.text())
            .then(str => new DOMParser().parseFromString(str, 'application/xml'))).documentElement;
    ...
    describe('EditorAction', () => {
        ...
        describe('invert', () => {
            ...
            it('inverts components of complex actions in reverse order', () => {
                ...
                action.actions.forEach((element, index) => expect(inverse.actions[inverse.actions.length - index - 1]).to.deep.equal(invert(action.actions[index])));
            });
            it('throws on unknown Action type', () => {
                const invalid = 'Not an action!';
    ...
    describe('isSame', () => {
        ...
        it('is true of any two Header Elements', () => {
            expect(isSame(scl1.querySelector('Header'), scl2.querySelector('Header'))).to.be.true;
        });
        it('is true of any two Communication Elements', () => {
            expect(isSame(scl1.querySelector('Communication'), scl2.querySelector('Communication'))).to.be.true;
        });
        it('is true of any two DataTypeTemplates Elements', () => {
            expect(isSame(scl1.querySelector('DataTypeTemplates'), scl2.querySelector('DataTypeTemplates'))).to.be.true;
        });
        ...
        it('is true of elements with equal nonempty id attributes', () => {
            expect(isSame(scl1.querySelector('LNodeType[id="Dummy.LLN0"]'), scl2.querySelector('LNodeType[id="Dummy.LLN0"]'))).to.be.true;
        });
        it('is false of elements with unequal id attributes', () => {
            expect(isSame(scl1.querySelector('LNodeType[id="Dummy.LLN0"]'), scl1.querySelector('LNodeType[id="Dummy.LLN0.two"]'))).to.be.false;
        });
    });
    describe('identity', () => {
        ...
        it('returns parent identity for singleton identities', () => {
            Object.entries(tags).forEach(([tag, data]) => {
                if (data.identity !== tags['Server'].identity)
                    return;
        ...
        it('returns valid identity for naming identities', () => {
            Object.entries(tags).forEach(([tag, data]) => {
                if (data.identity !== tags['Substation'].identity)
                    return;
                const element = scl1.querySelector(tag);
                if (element) {
                    expect(identity(element)).to.equal(identity(element.parentElement) +
    ...
    describe('find', () => {
        ...
        it('returns correct element for all tags except IEDName and ProtNs', () => {
            Object.keys(tags).forEach(tag => {
                const element = Array.from(scl1.querySelectorAll(tag)).filter(item => !item.closest('Private'))[0];
                if (element && tag !== 'IEDName' && tag !== 'ProtNs')
                    expect(element)
                        .to.satisfy((element) => element.isEqualNode(find(scl1, tag, identity(element))))
    ...
    describe('getReference', () => {
        it('returns correct reference for already existing elements', () => {
            Object.keys(tags)
                ...
                for (const childTag of childTags) {
                    expect(getReference(element, childTag)).to.equal(children.find(child => child.tagName === childTag));
                }
            });
        });
        it('returns correct reference for LNode element', () => {
            const scl = new DOMParser().parseFromString(`<Bay>
          <Private>testprivate</Private>
          <ConductingEquipment name="QA1"></ConductingEquipment>
        </Bay>`, 'application/xml').documentElement;
            expect(getReference(scl, 'LNode')).to.equal(scl.querySelector('ConductingEquipment'));
            const scl2 = new DOMParser().parseFromString(`<Bay>
          <Private>testprivate</Private>
          <PowerTransformer name="pTrans"></PowerTransformer>
          <ConductingEquipment name="QA1"></ConductingEquipment>
        </Bay>`, 'application/xml').documentElement;
            expect(getReference(scl2, 'LNode')).to.equal(scl2.querySelector('PowerTransformer'));
        });
        ...
        it('returns correct reference for VoltageLevel element', () => {
            const scl = new DOMParser().parseFromString(`<Substation>
          <Private></Private>
          <LNode></LNode>
        ...
        it('returns correct reference for Bay element', () => {
            const scl = new DOMParser().parseFromString(`<VoltageLevel>
          <Private></Private>
          <Function></Function>
        ...
        it('returns correct reference for ConductingEquipment element', () => {
          ...
          <ConnectivityNode></ConnectivityNode>
            ...
            expect(getReference(scl, 'ConductingEquipment')).to.equal(scl.querySelector('ConnectivityNode'));
        });
    });
    describe('findControlBlocks', () => {
        ...
        it('returns an Set of controlBlocks connected to the ExtRef', () => {
            const extRef = doc.querySelector(':root > IED[name="IED2"] > AccessPoint > Server > LDevice[inst="CircuitBreaker_CB1"] ExtRef');
            expect(findControlBlocks(extRef).size).to.have.equal(1);
            expect(Array.from(findControlBlocks(extRef))[0].isEqualNode(doc.querySelector('IED[name="IED1"] LDevice[inst="CircuitBreaker_CB1"] GSEControl[name="GCB"]'))).to.be.true;
        });
        ...
        it('returns empty array if input is not public', () => {
            expect(findControlBlocks(doc.querySelector('Private > ExtRef')).size).to.equal(0);
        });
    });
    describe('getUniqueElementName', () => {
        let parent;
        beforeEach(() => {
            const testDoc = new DOMParser().parseFromString('<Parent>' +
        ...
        it('returns unique name for Child', () => expect(getUniqueElementName(parent, 'Child')).to.equal('newChild3'));
        it('returns unique name for Child2', () => expect(getUniqueElementName(parent, 'Child2')).to.equal('newChild22'));
    });
    describe('getNameAttribute', () => {
        it('expect the correct value of the name attribute', () => {
            const doElement = scl1.querySelector('LNodeType[id="Dummy.LLN0"] > DO[type="Dummy.LLN0.Mod"]');
    ...
    describe('findFCDAs', () => {
        ...
        it('returns an array of FCDAs connected to the ExtRef', () => {
            ...
            expect(findFCDAs(extRef)[0].isEqualNode(doc.querySelector('IED[name="IED1"] LDevice[inst="CircuitBreaker_CB1"] ' +
        ...
        it('returns empty array if input is not public', () => {
            expect(findFCDAs(doc.querySelector('Private > ExtRef')).length).to.equal(0);
        });
    });
    describe('getChildElementsByTagName', () => {
        ...
        it('returns a child Element array with a specific tag', () => {
            ...
            expect(getChildElementsByTagName(parent, 'LNode').length).to.have.equal(parent?.querySelectorAll(':root > Substation > VoltageLevel > Bay[name="COUPLING_BAY"] > LNode').length);
        });
    });
    ...
    describe('depth', () => {
        ...
        it("returns the given object's or array's depth", () => expect(depth(fiveDeep)).to.equal(5));
        it('returns zero if given something other than an object or array', () => expect(depth('test')).to.equal(0));
        it('returns Infinity if given a circularly defined object or array', () => expect(depth(circular)).to.not.be.finite);
    });
    ...
    describe('generator function for new `lnInst` attribute', () => {
        ...
        describe('with existing unique lnInst', () => {
            ...
            it('returns unique lnInst called once', () => expect(lnInstGenerator('CSWI')).to.equal('3'));
        ...
        describe('with missing unique lnInst for lnClass PDIS', () => {
            beforeEach(() => {
                ...
                for (let i = 1; i <= 99; i++) {
                    const lNode = new DOMParser().parseFromString(`<LNode iedName="None" lnClass="PDIS" lnInst="${i}" />`, 'application/xml').documentElement;
                    parent.appendChild(lNode);
                }
                lnInstGenerator = newLnInstGenerator(parent);
            });
            it('return undefined for the lnClass PDIS', () => expect(lnInstGenerator('PDIS')).to.be.undefined);
            it('return unique lnInst for another lnClass', () => expect(lnInstGenerator('CSWI')).to.equal('1'));
        });
    });
    describe('minAvailableLogicalNodeInstance', () => {
        it('generates the minimum number not present yet as an "inst" attribute in a set of elements', () => {
            const docFragment = new DOMParser().parseFromString(`
      ...
      `, 'application/xml');
            let lnElements = Array.from(docFragment.querySelectorAll('LN[lnClass="LSVS"]'));
            expect(minAvailableLogicalNodeInstance(lnElements)).to.be.equal('2');
            lnElements = Array.from(docFragment.querySelectorAll('LN[lnClass="LLN0"]'));
