...
describe('filtered-list', () => {
    ...
    beforeEach(async () => {
        element = await fixture(html `<filtered-list multi
        >${Array.from(listItems).map(item => html `<mwc-check-list-item twoline ?disabled=${item.disabled}
              ><span>${item.prim}</span
    ...
    describe('has a check all checkbox that', () => {
        it('is indeterminate if one but not all check-list-items are selected', async () => {
            expect(element.shadowRoot.querySelector('mwc-formfield>mwc-checkbox')).to.not.have.attribute('indeterminate');
            element.items[0].click();
            await element.updateComplete;
            expect(element.shadowRoot.querySelector('mwc-formfield>mwc-checkbox')).to.have.attribute('indeterminate');
        });
        it('is selected if all check-list-items are selected', async () => {
            expect(element.shadowRoot.querySelector('mwc-formfield>mwc-checkbox')).to.not.have.attribute('checked');
            element.items
            ...
            await element.updateComplete;
            expect(element.shadowRoot.querySelector('mwc-formfield>mwc-checkbox')).to.have.attribute('checked');
        });
        ...
        it('selects all enabled and visible check-list-items on checkAll click', async () => {
            (element.shadowRoot.querySelector('mwc-formfield>mwc-checkbox')).click();
            await element.updateComplete;
            element.items
...
//# sourceMappingURL=filtered-list.test.js.map