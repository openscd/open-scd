...
describe('subnetwork-editor', () => {
    ...
    describe('with child GSE elements', () => {
        beforeEach(async () => {
            ...
            element = (await fixture(html `<subnetwork-editor
            .doc=${doc}
            .element=${subNetwork}
          ></subnetwork-editor>`));
        });
        it('looks like the latest snapshot', async () => await expect(element).shadowDom.to.equalSnapshot());
        it('has a name property', () => expect(element).to.have.property('name', 'StationBus'));
        ...
        it('has a desc property', () => expect(element).to.have.property('desc', 'desc'));
        it('has a type property', () => expect(element).to.have.property('type', '8-MMS'));
        it('return null with missing type attribute', async () => {
            subNetwork.removeAttribute('type');
            await element.requestUpdate();
            expect(element).to.have.property('type', null);
        });
        it('has a BitRate property', () => expect(element).to.have.property('bitrate', '100.0 b/s'));
...
//# sourceMappingURL=subnetwork-editor.test.js.map