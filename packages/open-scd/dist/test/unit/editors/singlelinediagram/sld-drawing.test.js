...
import { createBayElement, createBusBarElement, createConductingEquipmentElement, createConnectivityNodeElement, createPowerTransformerElement, createTerminalElement, createVoltageLevelElement, EQUIPMENT_SIZE, getAbsolutePosition, getBusBarLength, getDirections, getParentElementName, SVG_GRID_SIZE, } from '../../../../src/editors/singlelinediagram/sld-drawing.js';
...
describe('Single Line Diagram drawing', () => {
    ...
    describe('defines getDirections for equipment to connectivity node connection', () => {
        ...
        let cNode;
        beforeEach(() => {
            equipment = doc.querySelector('VoltageLevel[name="J1"] > Bay[name="Bay A"] > ConductingEquipment[name="QA1"]');
            cNode = doc.querySelector('VoltageLevel[name="J1"] > Bay[name="Bay A"] > ConnectivityNode[name="L2"]');
        });
    ...
    describe('defines a getBusBarLength function that', () => {
        it('returns a correct length for the bus bar given voltage level as root', () => {
            const element = doc.querySelector('VoltageLevel[name="J1"]');
            expect(getBusBarLength(element)).to.eql(18 * 2 * SVG_GRID_SIZE +
    ...
    describe('creates a group element for every given ConnectivityNode element that', () => {
        it('looks like its latest snapshot', async () => {
            const cNode = doc.querySelector('Bay[name="Bay A"] > ConnectivityNode[name="L1"]');
    ...
    describe('creates a group element for every given Terminal element that', () => {
        it('looks like its latest snapshot', async () => {
            const terminal = doc.querySelector('ConductingEquipment > Terminal[name="T2"]');
            await expect(createTerminalElement(terminal, 'top')).to.equalSnapshot();
        });
    });
});
//# sourceMappingURL=sld-drawing.test.js.map