...
import { attachedIeds, getAttachedIeds, } from '../../../../src/editors/substation/foundation.js';
describe('zeroline-pane', () => {
    ...
    it('per default looks like the latest snapshot', async () => {
        const element = await fixture(html `<zeroline-pane
        .doc=${doc}
        .getAttachedIeds="${undefined}"
      ></zeroline-pane>`);
        if (element.showieds.on)
            await element.showieds.click();
        if (element.showfunctions.on)
            await element.showfunctions.click();
    ...
    describe('attachedIeds', () => {
        ...
        describe('return IEDs for VoltageLevel elements that', () => {
            ...
            it('are not connected to another Bay ass well', async () => {
                expect((await attachedIeds(voltageLevel2, remainingIeds)).length).to.equal(0);
            });
        });
        describe('return IEDs for Substation elements that', () => {
            ...
            it('are not connected to another Substation ass well', async () => {
                expect((await attachedIeds(substation2, remainingIeds)).length).to.equal(0);
            });
        });
    });
    it('both the functions return every IED only once', async () => {
        ...
        const numUnRef = (await attachedIeds(doc.documentElement, remainingIeds))
            .length;
        const sumIeds = numBay1 + numBay2 + numVolt1 + numVolt2 + numSub1 + numSub2 + numUnRef;
