...
describe('wizards for DOType element', () => {
    ...
    describe('include an edit wizard that', () => {
        beforeEach(async () => {
            ...
            const wizard = dOTypeWizard(identity(doc.querySelector('DOType')), doc);
        ...
        describe('allows to edit id attribute', () => {
            ...
            it('that edits all referenced lnType attribute as well', () => {
                ...
                const numReferences = doc.querySelectorAll(`LNodeType > DO[type="${oldId}"], DOType > SDO[type="${oldId}"]`).length;
                input.value = 'someTestId';
                primaryAction.click();
                const complexAction = (actionEvent.args[0][0].detail.action);
                const actions = complexAction.actions;
...
//# sourceMappingURL=dotype.test.js.map