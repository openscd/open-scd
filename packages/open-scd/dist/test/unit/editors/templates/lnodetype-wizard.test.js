...
describe('wizards for LNodeType element', () => {
    ...
    describe('include an edit wizard that', () => {
        beforeEach(async () => {
            ...
            const wizard = lNodeTypeWizard(identity(doc.querySelector('LNodeType')), doc);
        ...
        describe('allows to edit lnClass attribute', () => {
            ...
            it('for valid input', async () => await fc.assert(fc.asyncProperty(regexString(regExp.lnClass, 4, 4), async (testValue) => {
        ...
        describe('allows to edit id attribute', () => {
            ...
            it('that edits all referenced lnType attribute as well', () => {
                ...
                const numReferences = doc.querySelectorAll(`LN0[lnType="${oldId}"], LN[lnType="${oldId}"]`).length;
                input.value = 'someTestId';
                primaryAction.click();
                const complexAction = (actionEvent.args[0][0].detail.action);
                const actions = complexAction.actions;
