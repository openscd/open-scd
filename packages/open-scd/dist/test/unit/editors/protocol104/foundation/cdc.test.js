...
import { createAddressElements, hasScaleFields, hasUnitMultiplierField, } from '../../../../../src/editors/protocol104/foundation/cdc.js';
describe('createAddressElements', () => {
    it('only single address element created without expected value', async () => {
        const ti = '30';
        const addressElements = createAddressElements(document, ti, false, undefined);
    ...
    it('only two address element created without expected value', async () => {
        const ti = '30';
        const addressElements = createAddressElements(document, ti, true, undefined);
    ...
    it('only single address element created with expected value', async () => {
        ...
        const addressElements = createAddressElements(document, ti, false, expectedValue);
        expect(addressElements.length).to.be.equal(1);
        expect(addressElements[0].tagName).to.be.equal('Address');
        expect(addressElements[0].getAttribute('ti')).to.be.equal(ti);
        expect(addressElements[0].getAttribute('expectedValue')).to.be.equal(expectedValue);
    });
    it('only two address element created without expected value', async () => {
        const ti = '30';
        const expectedValue = 'ev';
        const addressElements = createAddressElements(document, ti, true, expectedValue);
        expect(addressElements.length).to.be.equal(2);
        expect(addressElements[0].tagName).to.be.equal('Address');
        expect(addressElements[0].getAttribute('ti')).to.be.equal(ti);
        expect(addressElements[0].getAttribute('expectedValue')).to.be.equal(expectedValue);
        ...
        expect(addressElements[1].getAttribute('ti')).to.be.equal(ti);
        expect(addressElements[1].getAttribute('inverted')).to.be.equal('true');
        expect(addressElements[1].getAttribute('expectedValue')).to.be.equal(expectedValue);
    });
});
...
//# sourceMappingURL=cdc.test.js.map