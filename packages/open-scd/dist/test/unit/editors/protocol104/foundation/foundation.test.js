...
import { get104DetailsLine, getCdcValueFromDOIElement, getCtlModel, getDaElementByDaiElement, getDaiElement, getDaiValue, getDoElement, getEnumOrds, getEnumVal, getFullPath, getTypeAttribute, isEnumDataAttribute, } from '../../../../../src/editors/protocol104/foundation/foundation.js';
...
describe('foundation', () => {
    ...
    describe('get104DetailsLine', () => {
        ...
        it('returns basic fields', () => {
            const daiElement = doc.querySelector(`IED[name="B1"] LN0[lnType="SE_LLN0_SET_V001"] DAI[name="ctlVal"]`);
            const addressElement = daiElement.querySelector(FIRST_PRIV_ADDRESS_QUERY);
            expect(get104DetailsLine(daiElement, addressElement)).to.be.equal('casdu: 100, ioa: 4, ti: 62');
        });
        it('returns expectedValue fields', () => {
            const daiElement = doc.querySelector(`IED[name="B1"] LN0[lnType="SE_LLN0_SET_V001"] DOI[name="Health"] DAI[name="stVal"]`);
            const addressElement = daiElement.querySelector(FIRST_PRIV_ADDRESS_QUERY);
            expect(get104DetailsLine(daiElement, addressElement)).to.be.equal('casdu: 101, ioa: 1, ti: 30, expectedValue: 1 (Ok)');
        });
        it('returns check fields', () => {
            const daiElement = doc.querySelector(`IED[name="B1"] LN[lnType="SE_GGIO_SET_V002"] DOI[name="DPCSO1"] DAI[name="Check"]`);
            const addressElement = daiElement.querySelector(FIRST_PRIV_ADDRESS_QUERY);
            expect(get104DetailsLine(daiElement, addressElement)).to.be.equal('casdu: 202, ioa: 3, ti: 58, check: interlocking');
        });
        it('returns inverted fields', () => {
            const daiElement = doc.querySelector(`IED[name="B1"] LN[lnType="SE_GGIO_SET_V002"] DOI[name="Ind2"] DAI[name="stVal"]`);
            const addressElement = daiElement.querySelector(FIRST_PRIV_ADDRESS_QUERY);
            expect(get104DetailsLine(daiElement, addressElement)).to.be.equal('casdu: 1, ioa: 2, ti: 30, inverted: true');
        });
    });
    describe('getTypeAttribute', () => {
        it('expect the correct value of the type attribute', () => {
            const doElement = doc.querySelector('LNodeType[id="SE_GGIO_SET_V002"] > DO[name="Beh"]');
            expect(getTypeAttribute(doElement)).to.be.equal('SE_ENS_V001');
        });
    });
    describe('getFullPath', () => {
        it('returns expected value for DOI Element', () => {
            const doiElement = doc.querySelector('IED[name="B2"] LN0[lnType="SE_LLN0_SET_default_V001"] DOI[name="Beh"]');
            expect(getFullPath(doiElement, 'IED')).to.be.equal('AP1 / LD0 / LLN0 / Beh');
        });
    ...
    describe('getCdcValue', () => {
        ...
        it('returns expected value for CDC "ENC"', () => {
            // Basic test to see if CDC is retrieved correctly.
            const doiElement = doc.querySelector('IED[name="B1"] LN0[lnType="SE_LLN0_SET_V001"] DOI[name="Mod"]');
    ...
    describe('getCtlModel', () => {
        it('returns expected CtlModel Value', () => {
            const lnElement = doc.querySelector('IED[name="B1"] LN0[lnType="SE_LLN0_SET_V001"]');
    ...
    describe('getDaElementByDaiElement', () => {
        it('returns expected DA Element', () => {
            const daiElement = doc.querySelector('IED[name="B1"] LN0[lnType="SE_LLN0_SET_V001"] > DOI[name="Mod"] DAI[name="ctlVal"]');
        ...
        it('returns expected Enum DA Element', () => {
            const daiElement = doc.querySelector('IED[name="B1"] LN[lnType="SE_GGIO_SET_V002"] > DOI[name="ClcRfTyp"] > DAI[name="setVal"]');
            const daElement = getDaElementByDaiElement(daiElement);
            expect(daElement).to.be.not.null;
    ...
    describe('getEnumOrds', () => {
        ...
        it('returns correct list of Ord', () => {
            ...
            result.forEach((value, index) => expect(result[index]).to.be.equal('' + (index + 1)));
        });
    });
});
