...
import { addPrefixAndNamespaceToDocument, createPrivateAddress, createPrivateElement, getPrivateElement, PROTOCOL_104_NS, PROTOCOL_104_PREFIX, PROTOCOL_104_PRIVATE, } from '../../../../../src/editors/protocol104/foundation/private.js';
describe('foundation', () => {
    let doc;
    beforeEach(async () => {
    ...
    describe('addPrefixAndNamespaceToDocument', () => {
        it('adding prefix for namespace', async () => {
            ...
            expect(sclElement.getAttribute('xmlns:' + PROTOCOL_104_PREFIX)).to.be.equal(PROTOCOL_104_NS);
        });
    ...
    describe('getPrivateElement', () => {
        ...
        it('when called on DAI Element with Private then Private returned', () => {
            const daiElement = doc.querySelector('IED[name="B1"] LN0[lnType="SE_LLN0_SET_V001"] > DOI[name="Mod"] > SDI[name="Oper"] > DAI[name="ctlVal"]');
        ...
        it('when called on DAI Element without Private then null returned', () => {
            const daiElement = doc.querySelector('IED[name="B1"] LN0[lnType="SE_LLN0_SET_V001"] > DOI[name="Mod"] > DAI[name="ctlModel"]');
            const privateElement = getPrivateElement(daiElement);
    ...
    describe('createPrivateElement', () => {
        it('new private element is created with correct type', async () => {
            ...
            expect(privateElement.getAttribute('type')).to.be.equal(PROTOCOL_104_PRIVATE);
        });
    });
    describe('createPrivateElement', () => {
...
//# sourceMappingURL=private.test.js.map