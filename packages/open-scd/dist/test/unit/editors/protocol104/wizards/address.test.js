...
import { executeWizardReplaceAction, expectWizardNoUpdateAction, fetchDoc, setWizardSelectValue, setWizardTextFieldValue, } from '../../../wizards/test-support.js';
import { editAddressWizard, updateAddressValue, } from '../../../../../src/editors/protocol104/wizards/address.js';
describe('Wizards for 104 Address Element', () => {
    ...
    async function prepareWizard(daiQuery) {
    ...
    describe('when adding a 104 Address', () => {
        ...
        it('shows a validation error message if the combination of casdu and ioa is already in use', async () => {
            await setWizardTextFieldValue(inputs[2], '208'); // Casdu Field
            await setWizardTextFieldValue(inputs[3], '2'); // IOA Field
    ...
    describe('edit basic 104 Address', () => {
        beforeEach(async () => {
            await prepareWizard('IED[name="B2"] LN0[lnClass="LLN0"] DAI[name="stVal"] Address');
        });
        it('update basic fields should be updated in document', async function () {
            await setWizardTextFieldValue(inputs[2], '11'); // Casdu Field
            await setWizardTextFieldValue(inputs[3], '21'); // IOA Field
            const updateAction = executeWizardReplaceAction(updateAddressValue(doi, dai, address), element.wizardUI, inputs);
        ...
        it('when no fields changed there will be no update action', async function () {
            expectWizardNoUpdateAction(updateAddressValue(doi, dai, address), element.wizardUI, inputs);
        });
    ...
    describe('edit 104 Address with expected value', () => {
        beforeEach(async () => {
            await prepareWizard('IED[name="B1"] LN[lnType="SE_GGIO_SET_V002"] DOI[name="Mod"] DAI[name="ctlVal"] Address[ioa="2"]');
        });
        it('looks like the latest snapshot', async () => {
            await expect(element.wizardUI.dialog).dom.to.equalSnapshot();
        });
    ...
    describe('edit 104 Address with unit multiplier', () => {
        beforeEach(async () => {
            await prepareWizard('IED[name="B1"] LN[lnType="SE_GGIO_SET_V002"] DOI[name="IntIn1"] DAI[name="stVal"] Address');
        });
        it('update unit multiplier field should be updated in document', async function () {
            await setWizardSelectValue(inputs[5], 'k'); // Unit Multiplier Field
            const updateAction = executeWizardReplaceAction(updateAddressValue(doi, dai, address), element.wizardUI, inputs);
    ...
    describe('edit 104 Address with scale fields', () => {
        beforeEach(async () => {
            await prepareWizard('IED[name="B1"] LN[lnType="SE_MMXU_SET_V001"] DOI[name="Hz"] DAI[name="f"] Address');
        });
        it('update scale fields should be updated in document', async function () {
            await setWizardTextFieldValue(inputs[6], '1.234'); // Scale Multiplier Field
            await setWizardTextFieldValue(inputs[7], '2.345'); // Scale Offset Field
            const updateAction = executeWizardReplaceAction(updateAddressValue(doi, dai, address), element.wizardUI, inputs);
    ...
    describe('edit 104 Address with mapped cdc value', () => {
        beforeEach(async () => {
            await prepareWizard('IED[name="B1"] LN[lnType="SE_MMXU_SET_V001"] DOI[name="PPV"] DAI[name="f"] Address');
        });
        it('should have mappedCmv translation value in helper field', async () => {
            const cdc = element.wizardUI.dialog.querySelector('wizard-textfield[label="cdc"]');
            expect(cdc).to.exist;
    ...
    describe('edit 104 Address with inverted value', () => {
        beforeEach(async () => {
            await prepareWizard('IED[name="B1"] LN[lnType="SE_GGIO_SET_V002"] DOI[name="Ind2"] DAI[name="stVal"] Address[inverted="true"]');
        });
        it('looks like the latest snapshot', async () => {
            await expect(element.wizardUI.dialog).dom.to.equalSnapshot();
        });
    ...
    describe('edit 104 Address with check value', () => {
        beforeEach(async () => {
            await prepareWizard('IED[name="B1"] LN[lnType="SE_GGIO_SET_V002"] DOI[name="DPCSO1"] DAI[name="Check"] Address[check="interlocking"]');
        });
        it('looks like the latest snapshot', async () => {
            await expect(element.wizardUI.dialog).dom.to.equalSnapshot();
        });
