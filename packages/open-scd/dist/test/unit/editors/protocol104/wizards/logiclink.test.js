...
describe('Wizards for the Logic Link SCL element group', () => {
    ...
    describe('include an edit wizard that', () => {
        ...
        it('properly updates a P element of type IP', async () => {
            ...
            expect(action.old.element.textContent).to.eql('192.128.0.2');
            expect(action.new.element.textContent).to.eql('192.128.0.12');
        });
        it('properly creates a P element if not present', async () => {
            doc
                .querySelector('Communication > SubNetwork[name="W1"] > ConnectedAP[iedName="B1"] > Address > P[type="RG2-LL1-IP-SUBNET"]')
                ?.remove();
            input = (inputs.find(input => input.label === 'IP-SUBNET'));
            ...
            expect(action.new.element.textContent).to.eql('200.200.200.2');
        });
        it('properly deletes a full Logic Link group', async () => {
            const deleteAction = (element.wizardUI.dialog?.querySelector('mwc-menu > mwc-list-item'));
            deleteAction.click();
            await element.requestUpdate();
            const complexAction = actionEvent.args[0][0].detail.action;
            ...
            expect(firstAction.old.element.textContent).to.eql('192.128.0.2');
            const secondAction = complexAction.actions[1];
            expect(secondAction).to.satisfy(isDelete);
            expect(secondAction.old.element.textContent).to.eql('255.255.255.0');
        });
    });
    describe('include a create wizard that', () => {
        ...
        it("doesn't trigger a create editor action on primary action without all fields being filled in", async () => {
            const ipElement = (inputs.find(input => input.label === 'IP'));
        ...
        it('triggers a create editor action on primary action with all fields being filled in', async () => {
            const ipElement = (inputs.find(input => input.label === 'IP'));
            ipElement.value = '192.168.0.1';
            const ipSubNetElement = (inputs.find(input => input.label === 'IP-SUBNET'));
            ...
            expect(firstAction.new.element.textContent).to.eql('192.168.0.1');
            const secondAction = complexAction.actions[1];
            expect(secondAction).to.satisfy(isCreate);
            expect(secondAction.new.element.textContent).to.eql('255.255.255.0');
        });
    });
});
//# sourceMappingURL=logiclink.test.js.map