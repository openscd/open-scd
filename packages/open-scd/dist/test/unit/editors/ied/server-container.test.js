...
describe('server-container', () => {
    ...
    describe('Server Element with LDevice Elements', () => {
        let selectedLNClasses = [];
        beforeEach(async () => {
            serverElement = doc.querySelector('IED[name="IED1"] > AccessPoint[name="P1"] > Server');
        });
    ...
    describe('Server Element without LDevice Element', () => {
        beforeEach(async () => {
            serverElement = doc.querySelector('IED[name="IED5"] > AccessPoint[name="P1"] > Server');
            element = await fixture(html ` <server-container
        .element=${serverElement}
        .selectedLNClasses="${getLNClasses(serverElement)}"
      ></server-container>`);
...
//# sourceMappingURL=server-container.test.js.map