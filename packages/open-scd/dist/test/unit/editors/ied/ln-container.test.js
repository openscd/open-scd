...
describe('ln-container', async () => {
    ...
    it('looks like the latest snapshot with a LN0 element.', async () => {
      ...
      .element=${validSCL.querySelector('IED[name="IED1"] > AccessPoint[name="P1"] > Server > LDevice[inst="CircuitBreaker_CB1"] > LN0[lnClass="LLN0"]')}
      .nsdoc=${nsdoc}
    ></ln-container>`);
    ...
    it('looks like the latest snapshot with a LN element.', async () => {
        element = await fixture(html ` <ln-container
      .element=${validSCL.querySelector('IED[name="IED1"] > AccessPoint[name="P1"] > Server > LDevice[inst="CircuitBreaker_CB1"] > LN[lnClass="XCBR"]')}
      .nsdoc=${nsdoc}
    ></ln-container>`);
        await expect(element).shadowDom.to.equalSnapshot();
    });
    ...
    describe('has a getDOElements function ', () => {
        it('which return the DO containers underneath a given LN.', async () => {
            element = await fixture(html ` <ln-container
        .element=${validSCL.querySelector('IED[name="IED1"] > AccessPoint[name="P1"] > Server > LDevice[inst="Disconnectors"] > LN[lnClass="CILO"]')}
        .nsdoc=${nsdoc}
      ></ln-container>`);
        ...
        it('which return the DO containers underneath a given LN0.', async () => {
            element = await fixture(html ` <ln-container
        .element=${validSCL.querySelector('IED[name="IED2"] > AccessPoint[name="P1"] > Server > LDevice[inst="CircuitBreaker_CB1"] > LN0[lnClass="LLN0"]')}
        .nsdoc=${nsdoc}
      ></ln-container>`);
            const nestedDOs = element['getDOElements']();
            expect(nestedDOs).to.not.be.empty;
        ...
        it("which return an empty array if a LN doesn\t have child DO's.", async () => {
            element = await fixture(html ` <ln-container
        .element=${validSCL.querySelector('IED[name="IED2"] > AccessPoint[name="P1"] > Server > LDevice[inst="CircuitBreaker_CB1"] > LN[lnClass="tHarde"]')}
        .nsdoc=${nsdoc}
      ></ln-container>`);
            const nestedDOs = element['getDOElements']();
    ...
    describe('has a getInstanceElement function ', () => {
        beforeEach(async () => {
            element = await fixture(html ` <ln-container
        .element=${validSCL.querySelector('IED[name="IED2"] > AccessPoint[name="P1"] > Server > LDevice[inst="CBSW"] > LN[lnClass="XCBR"]')}
        .nsdoc=${nsdoc}
      ></ln-container>`);
        });
        it("which returns a DOI for a LN if it's available.", async () => {
            const dO = validSCL.querySelector('DataTypeTemplates > LNodeType[id="Dummy.XCBR1"] > DO[name="Pos"]');
        ...
        it('which returns null if no DOI is available.', async () => {
            const dO = validSCL.querySelector('DataTypeTemplates > LNodeType[id="Dummy.XCBR1"] > DO[name="Loc"]');
            expect(element['getInstanceElement'](dO)).to.be.null;
        });
    });
});
//# sourceMappingURL=ln-container.test.js.map