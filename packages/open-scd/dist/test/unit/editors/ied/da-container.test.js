...
describe('da-container', async () => {
    ...
    describe('with a DA element', () => {
        ...
        .element=${validSCL.querySelector('DataTypeTemplates > DOType[id="Dummy.XCBR1.Pos"] > DA[name="ctlModel"]')}
        .nsdoc=${nsdoc}
      ></da-container>`);
        });
        it('check the values returned for the header', () => {
            ...
            expect(header.values[2].values.length).to.be.equals(1);
            expect(header.values[2].values[0]).to.be.equals('CF');
        });
    ...
    describe('with a BDA element', () => {
        beforeEach(async () => {
            element = await fixture(html `<da-container
        .element=${validSCL.querySelector('DataTypeTemplates > DAType[id="Dummy.LPHD1.Sim.SBOw"] > BDA[name="ctlVal"]')}
        .daParent=${validSCL.querySelector('DataTypeTemplates > DOType[id="Dummy.LPHD1.Sim"] > DA[name="SBOw"]')}
        .ancestors=${[
                validSCL.querySelector('DataTypeTemplates > DOType[id="Dummy.LPHD1.Sim"] > DA[name="SBOw"]'),
            ]}
        .nsdoc=${nsdoc}
      ></da-container>`);
        });
    ...
    describe('with a BDA element having multiple values', () => {
        beforeEach(async () => {
            element = await fixture(html `<da-container
        .element=${validSCL.querySelector('DAType[id="ScaledValueConfig"] > BDA[name="scaleFactor"]')}
        .instanceElement=${validSCL.querySelector('IED[name="IED3"] DAI[name="scaleFactor"]')}
        .daParent=${validSCL.querySelector('DOType[id="DummySAV"] > DA[name="sVC"]')}
        .ancestors=${[
    ...
    describe('with a DA element and child elements are toggled', () => {
        beforeEach(async () => {
            element = await fixture(html `<da-container
        .element=${validSCL.querySelector('DataTypeTemplates > DOType[id="Dummy.LPHD1.Sim"] > DA[name="SBOw"]')}
        .nsdoc=${nsdoc}
      ></da-container>`);
            (element.shadowRoot.querySelector('mwc-icon-button-toggle')).click();
    ...
    it('looks like the latest snapshot with a DA element containing and a DAI', async () => {
        element = await fixture(html `<da-container
      .element=${validSCL.querySelector('DataTypeTemplates > DOType[id="Dummy.XCBR1.Pos"] > DA[name="ctlModel"]')}
      .instanceElement=${validSCL.querySelector(':root > IED[name="IED2"] > AccessPoint[name="P1"] > Server > LDevice[inst="CircuitBreaker_CB1"] > LN[lnType="Dummy.XCBR1"] > DOI[name="Pos"]> DAI[name="ctlModel"]')}
      .nsdoc=${nsdoc}
    ></da-container>`);
        await expect(element).shadowDom.to.equalSnapshot();
    });
    describe('has a getBDAElements function ', async () => {
      ...
      ></da-container>`);
        ...
        it('which returns no BDA elements if they are not available', async () => {
            element = await fixture(html `<da-container
        .element=${validSCL.querySelector('DataTypeTemplates > DOType[id="Dummy.LPHD1.Sim"] > DA[name="SBO"]')}
        .nsdoc=${nsdoc}
      ></da-container>`);
            const bdaElements = element['getBDAElements']();
...
//# sourceMappingURL=da-container.test.js.map