...
describe('extref-ln-binding-list', async () => {
    ...
    it('looks like the latest snapshot without a doc loaded', async () => {
        parent = await fixture(html ` <div class="container">
        <extref-ln-binding-list></extref-ln-binding-list>
      </div>`);
        element = (parent.querySelector('extref-ln-binding-list'));
    ...
    describe('for Sampled Value Control', () => {
        ...
        describe('when SVC has no subscriptions', () => {
            beforeEach(async () => {
                const svcElement = doc.querySelector('IED[name="SMV_Publisher"] SampledValueControl[name="fullSmv"]');
                const fcdaElement = doc.querySelector('IED[name="SMV_Publisher"] FCDA[ldInst="CurrentTransformer"][prefix="L2"][lnClass="TCTR"][lnInst="1"][doName="AmpSv"][daName="instMag.i"]');
        ...
        describe('when SVC has a single subscriptions', () => {
            beforeEach(async () => {
                const svcElement = doc.querySelector('IED[name="SMV_Publisher"] SampledValueControl[name="fullSmv"]');
                const fcdaElement = doc.querySelector('IED[name="SMV_Publisher"] FCDA[ldInst="CurrentTransformer"][prefix="L2"][lnClass="TCTR"][lnInst="1"][doName="AmpSv"][daName="q"]');
                element.dispatchEvent(newFcdaSelectEvent(svcElement, fcdaElement));
                await element.requestUpdate();
                await element.updateComplete;
            });
    ...
    describe('when SVC has a multiple subscriptions', () => {
        beforeEach(async () => {
            const svcElement = doc.querySelector('IED[name="SMV_Publisher"] SampledValueControl[name="fullSmv"]');
            const fcdaElement = doc.querySelector('IED[name="SMV_Publisher"] FCDA[ldInst="CurrentTransformer"][prefix="L3"][lnClass="TCTR"][lnInst="1"][doName="AmpSv"][daName="instMag.i"]');
            element.dispatchEvent(newFcdaSelectEvent(svcElement, fcdaElement));
            await element.requestUpdate();
            await element.updateComplete;
        });
        it('validate that some subscribed ExtRef Element will be disabled', async () => {
            const listItem = element.shadowRoot.querySelector('mwc-list-item[value="SMV_Subscriber2>>Overvoltage"]');
            expect(listItem).to.have.attribute('disabled');
        });
        it('validate that some available ExtRef Element will be disabled', async () => {
            const listItem = element.shadowRoot.querySelector('mwc-list-item[value="SMV_Subscriber2>>Overcurrent"]');
            expect(listItem).to.have.attribute('disabled');
        });
        it('expect the correct number of subscribed elements', () => {
    ...
    describe('for GOOSE Control', () => {
        ...
        describe('when GSEControl has no subscriptions', () => {
            beforeEach(async () => {
                const gseControlElement = doc.querySelector('IED[name="GOOSE_Publisher"] GSEControl[name="GOOSE1"]');
                const fcdaElement = doc.querySelector('IED[name="GOOSE_Publisher"] FCDA[ldInst="QB1_Disconnector"][prefix=""][lnClass="CSWI"][lnInst="1"][doName="Pos"][daName="stVal"]');
        ...
        describe('when GSEControl has a single subscription', () => {
            beforeEach(async () => {
                const gseControlElement = doc.querySelector('IED[name="GOOSE_Publisher"] GSEControl[name="GOOSE2"]');
                const fcdaElement = doc.querySelector('IED[name="GOOSE_Publisher"] FCDA[ldInst="QB2_Disconnector"][prefix=""][lnClass="CSWI"][lnInst="1"][doName="Pos"][daName="q"]');
                element.dispatchEvent(newFcdaSelectEvent(gseControlElement, fcdaElement));
                await element.requestUpdate();
                await element.updateComplete;
            });
    ...
    describe('when GSEControl has a multiple subscriptions', () => {
        beforeEach(async () => {
            const gseControlElement = doc.querySelector('IED[name="GOOSE_Publisher"] GSEControl[name="GOOSE2"]');
            const fcdaElement = doc.querySelector('IED[name="GOOSE_Publisher"] FCDA[ldInst="QB2_Disconnector"][prefix=""][lnClass="CSWI"][lnInst="1"][doName="Pos"][daName="stVal"]');
            element.dispatchEvent(newFcdaSelectEvent(gseControlElement, fcdaElement));
            await element.requestUpdate();
            await element.updateComplete;
        });
        it('validate that some subscribed ExtRef Element will be disabled', async () => {
            const listItem = element.shadowRoot.querySelector('mwc-list-item[value="GOOSE_Subscriber1>>Earth_Switch"]');
            expect(listItem).to.have.attribute('disabled');
        });
        it('validate that some available ExtRef Element will be disabled', async () => {
            const listItem = element.shadowRoot.querySelector('mwc-list-item[value="GOOSE_Subscriber1>>Earth_Switch> XSWI 1"]');
            expect(listItem).to.have.attribute('disabled');
        });
        it('expect the correct number of subscribed elements', () => {
...
//# sourceMappingURL=ext-ref-ln-binding-list.test.js.map