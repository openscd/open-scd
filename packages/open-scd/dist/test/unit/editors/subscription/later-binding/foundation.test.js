...
import { fcdaSpecification, inputRestriction, isSubscribedTo, sameAttributeValue, sameAttributeValueDiffName, } from '../../../../../src/editors/subscription/later-binding/foundation.js';
describe('smv-list', () => {
    let doc;
    beforeEach(async () => {
    ...
    describe('sameAttributeValue', () => {
        ...
        let extRef;
        beforeEach(async () => {
            fcda = doc.querySelector('IED[name="SMV_Publisher"] DataSet[name="currentOnlysDataSet"] FCDA[doName="AmpSv"][daName="instMag.i"]');
            extRef = doc.querySelector('IED[name="SMV_Subscriber"] LDevice[inst="Overvoltage"] ExtRef[doName="AmpSv"][daName="instMag.i"][srcCBName="currentOnly"]');
        });
    ...
    describe('sameAttributeValueDiffName', () => {
        ...
        let extRef;
        beforeEach(async () => {
            control = doc.querySelector('IED[name="SMV_Publisher"] SampledValueControl[name="currentOnly"]');
            extRef = doc.querySelector('IED[name="SMV_Subscriber"] LDevice[inst="Overvoltage"] ExtRef[doName="AmpSv"][daName="instMag.i"][srcCBName="currentOnly"]');
        });
        it('when both element do not have the attribute then they will be the same', () => {
            ...
            expect(sameAttributeValueDiffName(extRef, leftAttributeName, control, rightAttributeName)).to.be.true;
        });
        it('when both element have empty values then they will be the same', () => {
            const leftAttributeName = 'srcCBName';
            const rightAttributeName = 'name';
        ...
        it('when element have different values then they will be not the same', () => {
            ...
            expect(sameAttributeValueDiffName(extRef, leftAttributeName, control, rightAttributeName)).to.be.false;
        });
    });
    describe('isSubscribedTo', () => {
        ...
        describe('(2003 schema)', () => {
            beforeEach(async () => {
                ...
                fcda = doc.querySelector('IED[name="SMV_Publisher"] DataSet[name="currentOnlysDataSet"] FCDA[doName="AmpSv"][daName="instMag.i"]');
                control = doc.querySelector('IED[name="SMV_Publisher"] SampledValueControl[name="currentOnly"]');
                extRef = doc.querySelector('IED[name="SMV_Subscriber"] LDevice[inst="Overvoltage"] ExtRef[ldInst="CurrentTransformer"][doName="AmpSv"][daName="instMag.i"]');
            });
            it('when ExtRef and FDCA have same values then ExtRef is subscribed', () => {
                expect(isSubscribedTo(type, control, fcda, extRef)).to.be.true;
            });
...
describe('Input restriction', () => {
    ...
    beforeEach(async () => {
        doc = await fetch('/test/testfiles/editors/LaterBindingSMV2007B4.scd')
            .then(response => response.text())
            .then(str => new DOMParser().parseFromString(str, 'application/xml'));
        extRef = doc.querySelector('ExtRef[pLN="TCTR"][pDO="AmpSv"][pDA="instMag.i"]');
    });
    ...
    it('returns object with extracted CDC and bType', () => expect(inputRestriction(extRef)).to.deep.equal({
        cdc: 'SAV',
        bType: 'INT32',
    }));
});
describe('FCDA specification', () => {
    ...
    beforeEach(async () => {
        doc = await fetch('/test/testfiles/editors/LaterBindingSMV2007B4.scd')
            .then(response => response.text())
            .then(str => new DOMParser().parseFromString(str, 'application/xml'));
        fcda = doc.querySelector('FCDA[lnClass="TCTR"][doName="AmpSv"][daName="instMag.i"]');
    });
    it('returns null for missing logical node connection', () => {
    ...
    it('returns object with extracted CDC and bType', () => expect(fcdaSpecification(fcda)).to.deep.equal({
        cdc: 'SAV',
        bType: 'INT32',
    }));
});
