...
describe('goose-list', () => {
    ...
    it('triggers a newGOOSESelectEvent when a list item is clicked', () => {
        ...
        const listItem = Array.from(element.shadowRoot?.querySelectorAll('mwc-list-item[graphic="large"]') ??
            []).filter(a => a.innerHTML.includes('GCB'))[0];
        listItem.click();
        expect(selectEvent).to.have.be.calledOnce;
        expect(selectEvent.args[0][0].detail.gseControl).to.eql(validSCL.querySelector('IED[name="IED1"] > AccessPoint > Server > LDevice > LN0 > GSEControl[name="GCB"]'));
        expect(selectEvent.args[0][0].detail.dataset).to.eql(validSCL.querySelector('IED[name="IED1"] > AccessPoint > Server > LDevice > LN0 > DataSet[name="GooseDataSet1"]'));
    });
    it('triggers edit wizard when the Edit button of a GOOSE list item is pressed', async () => {
        ...
        (element.shadowRoot?.querySelector('mwc-list-item[graphic="large"]')).click();
        await element.updateComplete;
        expect(element.shadowRoot?.querySelector('mwc-list-item[graphic="large"] > mwc-icon-button[class=""]')).to.not.be.null;
        (element.shadowRoot?.querySelector('mwc-list-item[graphic="large"] > mwc-icon-button')).click();
        await element.updateComplete;
...
//# sourceMappingURL=goose-list.test.js.map