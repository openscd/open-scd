...
import { createExtRefElement, getExistingSupervision, getFirstSubscribedExtRef, instantiatedSupervisionsCount, updateExtRefElement, } from '../../../../src/editors/subscription/foundation.js';
...
describe('foundation', () => {
    ...
    describe('when using SCL Edition 2003 (1)', () => {
        ...
        it('when creating a ExtRef Element for a Sampled Value Control then correct attributes are filled', () => {
            const controlElement = doc.querySelector('IED[name="SMV_Publisher"] SampledValueControl[name="voltageOnly"]');
            const fcdaElement = doc.querySelector('IED[name="SMV_Publisher"] DataSet[name="voltageOnlysDataSet"] ' +
                '> FCDA[ldInst="VoltageTransformer"][prefix="L3"][lnClass="TVTR"][lnInst="1"][doName="VolSv"][daName="q"][fc="MX"]');
            const newExtRefElement = createExtRefElement(controlElement, fcdaElement);
            ...
            expect(newExtRefElement).to.have.attribute('ldInst', 'VoltageTransformer');
        ...
        it('when update a ExtRef Element for a Sampled Value Control then correct attributes are copied', () => {
            const extRefElement = doc.querySelector('IED[name="SMV_Subscriber"] ExtRef[intAddr="VolSv;TVTR3/VolSv/q"]');
            const controlElement = doc.querySelector('IED[name="SMV_Publisher"] SampledValueControl[name="voltageOnly"]');
            const fcdaElement = doc.querySelector('IED[name="SMV_Publisher"] DataSet[name="voltageOnlysDataSet"] ' +
                '> FCDA[ldInst="VoltageTransformer"][prefix="L3"][lnClass="TVTR"][lnInst="1"][doName="VolSv"][daName="q"][fc="MX"]');
            const clonedExtRefElement = updateExtRefElement(extRefElement, controlElement, fcdaElement);
            ...
            expect(clonedExtRefElement).to.have.attribute('ldInst', 'VoltageTransformer');
    ...
    describe('when using SCL Edition 2007B4 (2.1)', () => {
        ...
        it('when creating a ExtRef Element for a Sampled Value Control then correct attributes are filled', () => {
            ...
            expect(newExtRefElement).to.have.attribute('doName', 'VolSv');
            expect(newExtRefElement).to.have.attribute('daName', 'q');
            expect(newExtRefElement).to.have.attribute('srcLDInst', 'CurrentTransformer');
        ...
        it('when update a ExtRef Element for a Sampled Value Control then correct attributes are copied', () => {
            ...
            expect(clonedExtRefElement).to.have.attribute('doName', 'VolSv');
            expect(clonedExtRefElement).to.have.attribute('daName', 'q');
            expect(clonedExtRefElement).to.have.attribute('srcLDInst', 'CurrentTransformer');
    ...
    describe('when using SCL Edition 2007B4 with message binding and Sampled Values', () => {
        ...
        it('correctly locates the first ExtRef associated with a message', () => {
            const publishedControlBlock = doc.querySelector('IED[name="IED3"] SampledValueControl[name="MSVCB01"]');
            const subscribingIed = doc.querySelector('IED[name="IED1"]');
            const firstExtRef = getFirstSubscribedExtRef(publishedControlBlock, subscribingIed);
            expect(identity(firstExtRef)).to.be.equal('IED1>>CircuitBreaker_CB1>SMV:MSVCB01 MU01/ LLN0  IED3 MU01/I01A TCTR 1 Amp instMag.i');
        });
    ...
    describe('when using SCL Edition 2007B4 with later binding, SV and LSVS', () => {
        ...
        it('correctly identifies the existing number of supervision instances', () => {
            const controlBlock = doc.querySelector('IED[name="SMV_Publisher4"] SampledValueControl[name="voltageOnly"]');
    ...
    describe('when using SCL Edition 2007B4 with message binding and GOOSE', () => {
        ...
        it('correctly locates the first ExtRef associated with a message', () => {
            const publishedControlBlock = doc.querySelector('IED[name="IED2"] GSEControl[name="GCB"]');
            const subscribingIed = doc.querySelector('IED[name="IED1"]');
            const firstExtRef = getFirstSubscribedExtRef(publishedControlBlock, subscribingIed);
            expect(identity(firstExtRef)).to.be.equal('IED1>>Disconnectors>DC CSWI 1>GOOSE:GCB CBSW/ LLN0  IED2 CBSW/ XSWI 2 Pos stVal');
        });
    ...
    describe('when using SCL Edition 2007B4 with later binding, GOOSE and LGOS', () => {
        ...
        it('correctly identifies the existing number of supervision instances', () => {
            const controlBlock = doc.querySelector('IED[name="GOOSE_Publisher2"] GSEControl[name="GOOSE1"]');
