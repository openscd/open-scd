...
describe('ied-list', () => {
    ...
    it('triggers a newIEDSelectEvent when a list item is clicked', async () => {
        ...
        const listItem = Array.from(element.shadowRoot?.querySelectorAll('mwc-list-item') ?? []).filter(listItem => listItem.innerHTML.includes('IED2'))[0];
        listItem.click();
        await element.updateComplete;
        expect(selectEvent).to.have.be.calledOnce;
        expect(selectEvent.args[0][0].detail.ied).to.eql(validSCL.querySelector('IED[name="IED2"]'));
    });
});
//# sourceMappingURL=ied-list.test.js.map