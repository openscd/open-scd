...
function isAllMacUnique(parent, serviceType) {
    const allMacs = Array.from(parent.ownerDocument.querySelectorAll(`${serviceType} > Address > P[type="MAC-Address"]`)).map(pType => pType.textContent);
    const set = new Set(allMacs);
    return allMacs.length === set.size;
}
function isAllAppIdUnique(parent, serviceType) {
    const allMacs = Array.from(parent.ownerDocument.querySelectorAll(`${serviceType} > Address > P[type="APPID"]`)).map(pType => pType.textContent);
    const set = new Set(allMacs);
    return allMacs.length === set.size;
}
async function clickListItem(element, values) {
    Array.from(values).forEach(value => {
        element.wizardUI
            .dialog.querySelector(`mwc-check-list-item[value="${value}"]`)
            ?.click();
    });
...
describe('create wizard for ConnectedAP element', () => {
    ...
    it('looks like the latest snapshot', async () => await expect(element.wizardUI.dialog).dom.to.equalSnapshot());
    it('it does not allow to add already connected access points', () => {
        const disabledItems = Array.from(element.wizardUI.dialog.querySelectorAll('mwc-check-list-item')).filter(item => item.disabled);
        for (const item of disabledItems) {
            const [iedName, apName] = item.value.split('>');
            expect(doc.querySelector(`ConnectedAP[iedName="${iedName}"][apName="${apName}"]`)).to.exist;
        }
    });
    describe('on connecting one new access point', () => {
        it('adds a new ConnectedAP element', async () => {
            ...
            expect(parent.querySelector('ConnectedAP[iedName="GOOSE_Publisher"][apName="AP2"]')).to.exist;
        });
        describe('with publishing GSEControl or SampledValueControl', () => {
            it('create unique GSE for each GSEControl', async () => {
                await clickListItem(element, ['GOOSE_Publisher>AP2']);
                expect(parent.querySelectorAll('ConnectedAP[iedName="GOOSE_Publisher"][apName="AP2"] ' + '> GSE')).to.have.length(2);
            });
            ...
            it('create unique SMV for each SampledValueControl', async () => {
                ...
                expect(parent.querySelectorAll('ConnectedAP[iedName="SMV_Publisher"][apName="AP1"] ' + '> SMV')).to.have.length(2);
            });
    ...
    describe('on connecting multiple new access point', () => {
        it('adds new ConnectedAP element for each selected acc p', async () => {
            ...
            expect(parent.querySelector('ConnectedAP[iedName="GOOSE_Publisher"][apName="AP2"]')).to.exist;
            expect(parent.querySelector('ConnectedAP[iedName="GOOSE_Publisher"][apName="AP3"]')).to.exist;
        });
        describe('with publishing GSEControl or SampledValueControl', () => {
            it('create unique GSE for each GSEControl', async () => {
                await clickListItem(element, [
                ...
                expect(parent.ownerDocument.querySelectorAll('ConnectedAP[iedName="GOOSE_Publisher"]' + '> GSE')).to.have.length(3);
            });
            it('adds uniques GSE MAC-Address and APPID', async () => {
                await clickListItem(element, [
                    'GOOSE_Publisher>AP2',
            ...
            it('create unique SMV for each SampledValueControl', async () => {
                ...
                expect(parent.ownerDocument.querySelectorAll('ConnectedAP[iedName="SMV_Publisher"] ' + '> SMV')).to.have.length(3);
            });
...
//# sourceMappingURL=connectedap-c.test.js.map