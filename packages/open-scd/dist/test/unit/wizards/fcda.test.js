...
describe('create wizard for FCDA element', () => {
    ...
    describe('with a valid SCL file', () => {
        ...
        it('indicates error in case children cannot be determined', async () => {
            ...
            await element.requestUpdate();
            await finder.loaded;
            expect(element.wizardUI.dialog
                ?.querySelector('finder-list')
                ?.shadowRoot?.querySelector('p')?.innerText).to.equal('[error]');
        });
        describe('with a specific path', () => {
            ...
            it('returns a create action that follows the definition of a FCDA', () => {
                const newElement = (actionEvent.args[0][0].detail.action.new.element);
        ...
        describe('with path being non leaf node', () => {
            ...
            it('returns a non empty create action on primary action click', () => expect(actionEvent).to.not.have.been.called);
        });
        describe('with a incorrect logical node definition in the path', () => {
            ...
            it('does not return a empty action on primary action click', () => expect(actionEvent).to.not.have.been.called);
        });
...
//# sourceMappingURL=fcda.test.js.map