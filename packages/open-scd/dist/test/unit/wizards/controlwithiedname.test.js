...
describe('selectExtRefWizard', () => {
    ...
    it('shows all ExtRefs', async () => {
        ...
        await parent.updateComplete;
        await parent.wizardUI.updateComplete;
        expect(parent.wizardUI.dialog?.querySelectorAll('mwc-check-list-item').length).to.equal(14);
    });
    it('removes selected ExtRefs in case no intAddr is present', async () => {
        expect(doc.querySelector(':root > IED[name="IED2"] ExtRef[iedName="IED1"][ldInst="Disconnectors"][lnClass="XSWI"][doName="Pos"][daName="stVal"]')).to.exist;
        commMappings.items[3].click();
        await parent.updateComplete;
        await new Promise(resolve => setTimeout(resolve, 100)); // await animation
        ...
        await parent.updateComplete;
        expect(doc.querySelector(':root > IED[name="IED2"] ExtREf[iedName="IED1"][ldInst="Disconnectors"][lnClass="XSWI"][doName="Pos"][daName="stVal"]')).to.not.exist;
    });
    it('updates selected ExtRefs in case intAddr is present', async () => {
        expect(doc.querySelector(':root > IED[name="IED1"] ExtRef[iedName="IED2"][ldInst="CBSW"][lnClass="XSWI"]' +
        ...
        await parent.updateComplete;
        await new Promise(resolve => setTimeout(resolve, 100)); // await animation
        (parent.wizardUI.dialog?.querySelector('filtered-list')).items[0].click();
        await parent.updateComplete;
        (parent.wizardUI.dialog?.querySelector('mwc-button[slot="primaryAction"]')).click();
        await parent.updateComplete;
        expect(doc.querySelector(':root > IED[name="IED1"] ExtRef:not([iedName]):not([ldInst]):not([lnClass]):not([doName]):not([daName])' +
            '[intAddr="./stVal"][serviceType="GOOSE"][desc="testDesc"]' +
            '[pServT="GOOSE"][pLN="XSWI"][pDO="Pos"][pDA="stVal"]')).to.exist;
    });
    it('removes IEDName if all linked ExtRefs are removed/disconnected', async () => {
        expect(doc.querySelector(':root > IED[name="IED1"] > AccessPoint > Server > ' +
        ...
        (parent.wizardUI.dialog?.querySelector('filtered-list')).items[0].click();
        await parent.updateComplete;
        (parent.wizardUI.dialog?.querySelector('filtered-list')).items[1].click();
        await parent.updateComplete;
        (parent.wizardUI.dialog?.querySelector('filtered-list')).items[2].click();
        await parent.updateComplete;
        (parent.wizardUI.dialog?.querySelector('filtered-list')).items[3].click();
        await parent.updateComplete;
        (parent.wizardUI.dialog?.querySelector('mwc-button[slot="primaryAction"]')).click();
        await parent.updateComplete;
        expect(doc.querySelector(':root > IED[name="IED1"] > AccessPoint > Server > ' +
...
//# sourceMappingURL=controlwithiedname.test.js.map