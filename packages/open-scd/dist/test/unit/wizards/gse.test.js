...
import { isCreate, isDelete, isSimple, isReplace, } from '../../../src/foundation.js';
import { editGseWizard, getMTimeAction, updateGSEAction, } from '../../../src/wizards/gse.js';
describe('gse wizards', () => {
    ...
    describe('updateGSEAction', () => {
        ...
        beforeEach(async () => {
            ...
            wizard = editGseWizard(doc.querySelector('GSE[ldInst="CircuitBreaker_CB1"][cbName="GCB"]'));
            element.workflow.push(() => wizard);
            await element.requestUpdate();
        ...
        it('does not update a GSE element when no attribute has changed', () => {
            ...
            const actions = editorAction(inputs, element.wizardUI)[0]
                .actions;
        ...
        it('update a GSE element when only MAC-Address attribute changed', async () => {
            ...
            const actions = complexAction[0].actions;
            ...
            expect(oldElement.querySelector('P[type="MAC-Address"]')?.textContent?.trim()).to.equal('01-0C-CD-01-00-10');
            expect(newElement.querySelector('P[type="MAC-Address"]')?.textContent?.trim()).to.equal('01-0C-CD-01-00-11');
        });
        it('update a GSE element when only APPID attribute changed', async () => {
            ...
            const oldElement = actions[0].old.element;
            const newElement = actions[1].new.element;
            expect(oldElement.querySelector('P[type="APPID"]')?.textContent?.trim()).to.equal('0010');
            expect(newElement.querySelector('P[type="APPID"]')?.textContent?.trim()).to.equal('014');
        });
        it('update a GSE element when only VLAN-ID attribute changed', async () => {
            ...
            expect(oldElement.querySelector('P[type="VLAN-ID"]')?.textContent?.trim())
                .to.be.undefined;
            expect(newElement.querySelector('P[type="VLAN-ID"]')?.textContent?.trim()).to.equal('0F1');
        });
        it('update a GSE element when only VLAN-PRIORITY attribute changed', async () => {
            ...
            const oldElement = actions[0].old.element;
            const newElement = actions[1].new.element;
            expect(oldElement.querySelector('P[type="VLAN-PRIORITY"]')?.textContent?.trim()).to.equal('4');
            expect(newElement.querySelector('P[type="VLAN-PRIORITY"]')?.textContent?.trim()).to.equal('7');
        });
    ...
    describe('getMTimeAction', () => {
        const gse = new DOMParser().parseFromString(`<GSE ldInst="myLdInst" cbName="cbName"></GSE>`, 'application/xml').documentElement;
        const oldMinTime = new DOMParser().parseFromString(`<MinTime unit="s" multiplier="m">10</MinTime>`, 'application/xml').documentElement;
        const oldMaxTime = new DOMParser().parseFromString(`<MaxTime unit="s" multiplier="m">10000</MaxTime>`, 'application/xml').documentElement;
        it('updates a MinTime child element when chenged', () => {
            ...
            expect(editorAction.new.element.textContent?.trim()).to.equal('654');
        });
        ...
        it('updates a MaxTime child element when chenged', () => {
            const editorAction = getMTimeAction('MaxTime', oldMaxTime, '1234123', gse);
            expect(editorAction).to.satisfy(isReplace);
            expect(editorAction.new.element.textContent?.trim()).to.equal('1234123');
        });
...
//# sourceMappingURL=gse.test.js.map