...
import { createProcessWizard, editProcessWizard, } from '../../../src/wizards/process.js';
describe('Wizards for SCL Process element', () => {
    ...
    describe('define an edit wizard that', () => {
        beforeEach(async () => {
            const wizard = editProcessWizard(doc.querySelector('Process[name="ProcessGenConduct"]'));
        ...
        it('allows to create non required attribute type', async () => {
            ...
            expect(action).to.satisfy(isReplace);
            const editAction = action;
            expect(editAction.new.element).to.have.attribute('type', 'someNonEmptyType');
        });
    });
    describe('define a create wizard that', () => {
        beforeEach(async () => {
            const wizard = createProcessWizard(doc.querySelector('Process[name="ProcessGenConduct"]'));
            element.workflow.push(() => wizard);
            await element.requestUpdate();
            inputs = Array.from(element.wizardUI.inputs);
            primaryAction = (element.wizardUI.dialog?.querySelector('mwc-button[slot="primaryAction"]'));
        ...
        it('allows to create name and non required attribute type', async () => {
            ...
            expect(action).to.satisfy(isCreate);
            const createAction = action;
            expect(createAction.new.element).to.have.attribute('type', 'someNonEmptyType');
        });
    });
});
