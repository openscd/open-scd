...
import { isCreate, isReplace, isDelete, isSimple, } from '../../../src/foundation.js';
import { createAction, updateAction, } from '../../../src/wizards/voltagelevel.js';
describe('VoltageLevelEditor', () => {
    ...
    let inputs;
    beforeEach(async () => {
        inputs = await Promise.all(['name', 'desc', 'nomFreq', 'numPhases', 'Voltage'].map(label => (fixture(html `<wizard-textfield label=${label}></wizard-textfield>`))));
    });
    ...
    describe('with no nulled properties', () => {
        describe('has a createAction that', () => {
            let parent;
            beforeEach(() => {
                parent = new DOMParser().parseFromString('<Substation></Substation>', 'application/xml').documentElement;
            });
        ...
        describe('has an updateAction that', () => {
            describe('with missing child element Voltage', () => {
                let element;
                beforeEach(() => {
                    element = new DOMParser().parseFromString('<VoltageLevel></VoltageLevel>', 'application/xml').documentElement;
                });
            ...
            describe('with no change in element VoltageLevel but changes in the child element Voltage', () => {
                let element;
                beforeEach(() => {
                    element = new DOMParser().parseFromString(`<VoltageLevel name="" desc="" nomFreq="" numPhases="">
              <Voltage unit="V" multiplier="k">110</Voltage>
              </VoltageLevel>`, 'application/xml').documentElement;
                });
            ...
            describe('with no change in VoltageLevel nor Voltage', () => {
                let element;
                beforeEach(() => {
                    element = new DOMParser().parseFromString('<VoltageLevel name="" desc="" nomFreq="" numPhases=""><Voltage unit="V"></Voltage></VoltageLevel>', 'application/xml').documentElement;
                });
...
//# sourceMappingURL=voltagelevel.test.js.map