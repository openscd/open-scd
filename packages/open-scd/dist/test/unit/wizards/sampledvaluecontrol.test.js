...
import { isCreate, isDelete, isReplace, isSimple, } from '../../../src/foundation.js';
import { createSampledValueControlWizard, editSampledValueControlWizard, removeSampledValueControlAction, selectSampledValueControlWizard, } from '../../../src/wizards/sampledvaluecontrol.js';
...
describe('Wizards for SCL element SampledValueControl', () => {
    ...
    describe('define an edit wizard that', () => {
        describe('with muticast attribute set to false - deprecated', () => {
            beforeEach(async () => {
                const wizard = editSampledValueControlWizard(doc.querySelector('SampledValueControl[multicast="false"]'));
            ...
            it('update a SampledValueControl element when only multicast attribute changed', async () => {
                ...
                expect(updateAction.old.element).to.have.attribute('multicast', 'false');
                expect(updateAction.new.element).to.have.attribute('multicast', 'true');
            });
        });
        describe(' with multicast set to treu', () => {
            beforeEach(async () => {
                const wizard = editSampledValueControlWizard(doc.querySelector('SampledValueControl[multicast="true"]'));
                element.workflow.push(() => wizard);
                await element.requestUpdate();
                inputs = Array.from(element.wizardUI.inputs);
                primaryAction = (element.wizardUI.dialog?.querySelector('mwc-button[slot="primaryAction"]'));
            ...
            it('edits name attribute only for valid inputs', async () => {
                await fc.assert(fc.asyncProperty(regexString(regExp.tAsciName, 1, 32), async (name) => {
                    inputs[0].value = name;
            ...
            it('rejects smpRate attribute in case input is not unsigned int', async () => {
                await fc.assert(fc.asyncProperty(regexString(inverseRegExp.uint, 1), async (smpRate) => {
                    inputs[4].value = smpRate;
                    await inputs[4].requestUpdate();
            ...
            it('edits nofASDU attribute only for valid inputs', async () => {
                ...
                await fc.assert(fc.asyncProperty(integer({ min: 0 }).map(num => `${num}`), async (nofASDU) => {
                    input.value = nofASDU;
                    await input.requestUpdate();
            ...
            it('rejects nofASDU attribute in case input is not unsigned int', async () => {
                ...
                input.nullSwitch?.click();
                await input.requestUpdate();
                await fc.assert(fc.asyncProperty(regexString(inverseRegExp.uint, 1), async (nofASDU) => {
                    input.value = nofASDU;
                    await input.requestUpdate();
            ...
            it('update a SampledValueControl element when smvID attribute changed', async () => {
                ...
                expect(action).to.satisfy(isReplace);
                const updateAction = action;
                expect(updateAction.old.element).to.have.attribute('smvID', 'some/reference');
                expect(updateAction.new.element).to.have.attribute('smvID', 'myNewType/ID');
            });
            ...
            it('updates the SampledValueEnable element when securityEnabled changed', async () => {
                ...
                expect(updateAction.new.element).to.have.attribute('securityEnabled', 'Signature');
            });
        });
        describe('contains a remove button that', () => {
            const ln01smv = new DOMParser().parseFromString(`<LN0 lnClass="LLN0" lnType="myType">
            ...
            const ln02smv2 = new DOMParser().parseFromString(`<LN0 lnClass="LLN0" lnType="myType">
                <DataSet name="myDataSet"/>
                <SampledValueControl name="myName" datSet="myDataSet"/>
                <SampledValueControl name="myName2" datSet="myDataSet"/>
            </LN0>`, 'application/xml').documentElement;
            const ln02gse = new DOMParser().parseFromString(`<LN0 lnClass="LLN0" lnType="myType">
                  <DataSet name="myDataSet"/>
                  <SampledValueControl name="myName" datSet="myDataSet"/>
                  <GSEControl name="myName2" datSet="myDataSet"/>
              </LN0>`, 'application/xml').documentElement;
            const ln02rp = new DOMParser().parseFromString(`<LN0 lnClass="LLN0" lnType="myType">
              <DataSet name="myDataSet"/>
              <ReportControl name="myName" datSet="myDataSet"/>
              <SampledValueControl name="myName2" datSet="myDataSet"/>
          </LN0>`, 'application/xml').documentElement;
            const missingparent = (new DOMParser().parseFromString(`<SampledValueControl name="myName" datSet="myDataSet"/>`, 'application/xml').documentElement);
            it('removes SampledValueControl and its referenced DataSet if no other SampledValueControl is assigned', () => {
                ...
                const actions = (removeSampledValueControlAction(sampledValueControl)?.actions);
            ...
            it('does not remove DataSet with another SampledValueControl referenced', () => {
                const sampledValueControl = ln02smv2.querySelector('SampledValueControl');
                const actions = (removeSampledValueControlAction(sampledValueControl)?.actions);
            ...
            it('removes referenced SMV element in the Communication section', () => {
                const sampledValueControl = doc.querySelector('IED[name="IED3"] SampledValueControl');
                const actions = (removeSampledValueControlAction(sampledValueControl)?.actions);
                ...
                expect(actions[2].old.element).to.equal(doc.querySelector('Communication SMV[ldInst="MU01"][cbName="MSVCB01"]'));
            });
        });
    });
    describe('define an create wizard that', () => {
        ...
        describe('with existing ConnectedAP element in the Communication section', () => {
            beforeEach(async () => {
                ...
                dataPicker = (element.wizardUI.dialogs[3]?.querySelector('finder-list'));
                await element.wizardUI.requestUpdate(); // make sure wizard is rendered
            });
            it('has fourth pages', () => expect(element.wizardUI.dialogs.length).to.equal(4));
            ...
            it('complex action carries SampledValueControl element', async () => {
                ...
                expect(createAction.new.element.tagName).to.equal('SampledValueControl');
            });
            ...
            it('referenced DataSet element not having any FCDA per default', async () => {
                await primaryAction.click();
                const createAction = (actionEvent.args[0][0].detail.action.actions[2]);
            ...
            it('complex action adding SMV element in the Communication section', async () => {
                ...
                expect(action).to.satisfy(isCreate);
                const createAction = action;
                expect(createAction.new.element.tagName).to.equal('SMV');
            });
        });
        describe('with missing ConnectedAP element in the Communication section', () => {
            beforeEach(async () => {
                const wizard = createSampledValueControlWizard(doc.querySelector('IED[name="IED4"] LN0'));
                element.workflow.push(() => wizard);
                await element.requestUpdate();
                (element.wizardUI.dialogs[0].querySelector('wizard-textfield[label="smvID"]')).maybeValue = 'wer';
                primaryAction = (element.wizardUI.dialogs[3]?.querySelector('mwc-button[slot="primaryAction"]'));
...
//# sourceMappingURL=sampledvaluecontrol.test.js.map