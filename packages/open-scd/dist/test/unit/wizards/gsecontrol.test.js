...
import { contentGseControlWizard, createGseControlWizard, editGseControlWizard, gseControlParentSelector, removeGseControlAction, selectGseControlWizard, updateGseControlAction, } from '../../../src/wizards/gsecontrol.js';
...
describe('gsecontrol wizards', () => {
    ...
    describe('removeGseControl', () => {
        ...
        const missingparent = (new DOMParser().parseFromString(`<GSEControl name="myName" datSet="myDataSet"/>`, 'application/xml').documentElement);
        it('removes GSEControl and its refereced DataSet if no other GSEControl are aasinged', () => {
            ...
            const actions = removeGseControlAction(gseControl).actions;
        ...
        it('removes GSE element if present in the Communication section', () => {
            ...
            expect(actions[2].old.element).to.equal(doc.querySelector('Communication GSE[ldInst="CircuitBreaker_CB1"][cbName="GCB"]'));
        });
    });
    describe('updateGseControlAction', () => {
        const gseControl = (new DOMParser().parseFromString(`<GSEControl name="myCbName" type="GOOSE" datSet="myDataSet" appID="myAPP/ID"></GSEControl>`, 'application/xml').documentElement);
        let inputs;
        let wizard;
        const noOp = () => {
            return;
        ...
        it('update a GSEControl element when appID attribute changed', async () => {
            ...
            expect(updateAction.new.element).to.have.attribute('appID', 'myNewType/ID');
        });
        ...
        it('update a GSEControl element when securityEnabled attribute changed', async () => {
            ...
            expect(updateAction.new.element).to.have.attribute('securityEnabled', 'SignatureAndEncryption');
        });
    });
    describe('define an create wizard that', () => {
        ...
        describe('with existing ConnectedAP element in the Communication section', () => {
            beforeEach(async () => {
                ...
                primaryAction = (element.wizardUI.dialogs[2]?.querySelector('mwc-button[slot="primaryAction"]'));
                dataPicker = (element.wizardUI.dialogs[2]?.querySelector('finder-list'));
                await element.wizardUI.requestUpdate(); // make sure wizard is rendered
            });
            it('has three pages', () => expect(element.wizardUI.dialogs.length).to.equal(3));
            ...
            it('complex action carries GSEControl element', async () => {
                ...
                expect(createAction.new.element.tagName).to.equal('GSEControl');
            });
            ...
            it('referenced DataSet element not having any FCDA per default', async () => {
                ...
                actions.forEach(action => {
                    expect(action).to.satisfy(isCreate);
                    const createAction = action;
                    expect(createAction.new.element.tagName).to.not.equal('FCDA');
                });
            });
            it('referenced DataSet element saving selected FCDA', async () => {
                ...
                expect(action).to.satisfy(isCreate);
                const createAction = action;
                expect(createAction.new.element.tagName).to.equal('FCDA');
            });
            it('complex action adding GSE element in the Communication section', async () => {
                ...
                expect(action).to.satisfy(isCreate);
                const createAction = action;
                expect(createAction.new.element.tagName).to.equal('GSE');
            });
        });
        describe('with missing ConnectedAP element in the Communication section', () => {
            beforeEach(async () => {
                const wizard = createGseControlWizard(doc.querySelector('IED[name="IED4"] LN0'));
                element.workflow.push(() => wizard);
                await element.requestUpdate();
                (element.wizardUI.dialogs[0].querySelector('wizard-textfield[label="appID"]')).maybeValue = 'wer';
                primaryAction = (element.wizardUI.dialogs[2]?.querySelector('mwc-button[slot="primaryAction"]'));
...
//# sourceMappingURL=gsecontrol.test.js.map