...
import { getSinkReferences, getSourceReferences, } from '../../../src/wizards/commmap-wizards.js';
describe('communication mapping wizard', () => {
    ...
    describe('getSinkReferences', () => {
        it('returns an array of all ClientLN`s for doc as input', () => {
            ...
            expect(clientlns[0].isEqualNode(doc.querySelector('ReportControl[name="ReportCb"] ClientLN')))?.to.be.true;
        });
        it('returns an array all ClientLN`s connected to an IED - send and receive', () => {
            const clientlns = getSinkReferences(doc.querySelector('IED[name="IED1"]'));
    ...
    describe('getSourceReferences', () => {
        ...
        it('returns an array of all ExtRef`s connected to an IED - send and receive', () => {
            const extRefs = getSourceReferences(doc.querySelector('IED[name="IED2"]'));
            expect(extRefs).to.have.length(6);
            expect(extRefs[2].isEqualNode(doc.querySelector('IED[name="IED2"] ExtRef'))).to.be.true;
        });
    });
});
//# sourceMappingURL=commmap.test.js.map