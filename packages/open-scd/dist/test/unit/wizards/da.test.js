...
import { createDaAction, renderDa, updateDaAction, } from '../../../src/wizards/da.js';
describe('da wizards', () => {
    describe('updateDaAction', () => {
        let doc;
        let data;
        let element;
        const da = (new DOMParser().parseFromString(`<DA name="ctlModel" bType="Enum" type="Dummy_ctlModel" fc="CF"></DA>`, 'application/xml').documentElement);
        let inputs;
        let wizard;
        const noOp = () => {
            return;
            ...
            wizard = [
                {
                    title: 'title',
                    content: [
                        ...wizardContent('ctlModel', null, 'Enum', types, 'Dummy_ctlModel', null, null, null, null, data),
        ...
        it('creates a Val child Val attribute changes from null to something', async () => {
            ...
            expect(updateAction.new.element
                .querySelector('Val')
                ?.textContent?.trim()).to.not.equal('direct-with-normal-security');
        });
        it('update a DA element when fc attribute changed', async () => {
            const input = inputs[9];
            input.value = 'ST';
            await input.requestUpdate();
            const editorAction = updateDaAction(da);
            const updateActions = editorAction(inputs, newWizard());
    ...
    describe('createDaAction', () => {
        ...
        let data;
        let element;
        const daType = (new DOMParser().parseFromString(`<DOType id="myID"></DOType>`, 'application/xml').documentElement);
        let inputs;
        let wizard;
        const noOp = () => {
            return;
            ...
            wizard = [
                {
                    title: 'title',
                    content: [
                        ...wizardContent('sboTimeout', null, 'INT32', types, null, null, null, null, null, data),
                        ...renderDa('CF', null, null, null),
                    ],
                },
            ];
        ...
        it('creates yet another BDA element with different attribute setting', async () => {
            ...
            const dchg = inputs[10];
            const qchg = inputs[11];
            const dupd = inputs[12];
...
//# sourceMappingURL=da.test.js.map