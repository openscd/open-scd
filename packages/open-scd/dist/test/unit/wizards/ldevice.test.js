...
import { createUpdateAction, getValue, } from '../../../src/foundation.js';
import { editLDeviceWizard } from '../../../src/wizards/ldevice.js';
import { fetchDoc, setWizardTextFieldValue, expectUpdateAction, } from './test-support.js';
describe('Wizards for SCL element LDevice', () => {
    ...
    it('contains a wizard-textfield with a non-empty "inst" value', async () => {
        expect(inputs.find(textField => textField.label == 'ldInst')
            ?.value).to.be.equal(ldevice.getAttribute('inst'));
    });
    it('contains a wizard-textfield with an empty "desc" value', async () => {
        expect(inputs.find(textField => textField.label == 'desc')
            ?.value).to.be.equal('');
    });
    it('contains a wizard-textfield with an empty "ldName" value', async () => {
        expect(inputs.find(textField => textField.label == 'ldName')
            ?.value).to.be.equal('');
    });
    describe('Allowing/Disallowing ldName editing', () => {
        it('ConfLdName should not be present and therefore ldName should be readonly', async function () {
            ...
            expect(inputs[0]).to.have.attribute('readonly');
        });
        ...
        it('ConfLdName should be present in IED1 and therefore ldName should not be readonly', async function () {
            ...
            expect(inputs[0]).to.not.have.attribute('readonly');
            describe('Modify ldName', () => {
                it('should be registered as an update action', async () => {
                    const newValue = 'LDevice1';
                    const ldeviceTextField = inputs.find(textField => textField.label == 'ldName');
                    ...
                    expectUpdateAction(simpleAction, ldevice.tagName, ldeviceTextField.label, null, newValue);
                });
            });
        });
    });
});
//# sourceMappingURL=ldevice.test.js.map