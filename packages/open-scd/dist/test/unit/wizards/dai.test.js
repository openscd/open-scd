...
import { createDAIWizard, createValue, editDAIWizard, updateValue, } from '../../../src/wizards/dai.js';
describe('Wizards for SCL element DAI', () => {
    ...
    describe('create DAI with existing Val Element', () => {
        ...
        it('update value should be updated in document', async function () {
            await setWizardTextFieldValue(inputs[0], '24');
            const complexActions = createValue(doi, da, dai, dai)(inputs, element.wizardUI);
    ...
    describe('create DAI without Val Element', () => {
        ...
        it('update value should be updated in document', async function () {
            await setWizardTextFieldValue(inputs[0], '');
            await setWizardTextFieldValue(inputs[1], '');
            const complexActions = createValue(doi, da, dai, dai)(inputs, element.wizardUI);
    ...
    describe('edit existing DAI with Val Element', () => {
        ...
        describe('having multiple setting group values', () => {
            ...
            it('update value should be updated in document', async function () {
                await setWizardTextFieldValue(inputs[0], '800');
                const complexActions = updateValue(da, val)(inputs, element.wizardUI);
                expectUpdateComplexAction(complexActions);
                const replace = complexActions[0].actions[0];
    ...
    function expectUpdateComplexAction(complexActions) {
        ...
        expect(complexActions[0].title).to.equal('[dai.action.updatedai]');
        const actions = complexActions[0].actions;
        expect(actions.length).to.equal(1);
    }
    function expectCreateComplexAction(complexActions, expectedValue) {
        expect(complexActions.length).to.equal(1);
        expect(complexActions[0]).to.not.satisfy(isSimple);
        expect(complexActions[0].title).to.equal('[dai.action.createdai]');
        const actions = complexActions[0].actions;
        expect(actions.length).to.equal(1);
