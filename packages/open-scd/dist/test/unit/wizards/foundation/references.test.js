import { expectDeleteAction, expectReplaceAction, expectUpdateTextValue, fetchDoc, } from '../test-support.js';
import { deleteReferences, updateReferences, } from '../../../../src/wizards/foundation/references.js';
...
describe('Update reference for ', () => {
    ...
    describe('IED', () => {
        ...
        it('will update all references to IED IED1', function () {
            ...
            expectReplaceAction(updateActions[0], 'Association', 'iedName', oldName, newName);
            expectReplaceAction(updateActions[1], 'ClientLN', 'iedName', oldName, newName);
        });
        it('will update all references to IED IED2', function () {
            ...
            expectUpdateTextValue(updateActions[7], 'SampledValueControl', oldName, newName);
        });
        it('will update all references to IED Pub and checks for correct Val elements', function () {
            ...
            const valSuffix3 = input3.getAttribute('srcLDInst') +
                '/' +
                input3.getAttribute('srcLNClass') +
                '.' +
                input3.getAttribute('srcCBName');
            expectUpdateTextValue(updateActions[4], 'DAI', oldName + valSuffix3, newName + valSuffix3);
        });
    ...
    describe('Substation', () => {
        ...
        it('will update all references to Substation AA1', function () {
            ...
            expectReplaceAction(updateActions[0], 'Terminal', 'substationName', oldName, newName);
        });
    });
    describe('VoltageLevel', () => {
        ...
        it('will update all references to VoltageLevel "J1"', function () {
            ...
            expect(updateActions.length).to.equal(48);
            expectReplaceAction(updateActions[0], 'Terminal', 'voltageLevelName', oldName, newName);
        });
    });
    describe('Bay', () => {
        beforeEach(async () => {
        ...
        it('will update all references to BusBar "BusBar A"', function () {
            ...
            expectReplaceAction(updateActions[0], 'Terminal', 'bayName', oldName, newName);
        });
