...
import { reportControlParentSelector, createReportControlWizard, editReportControlWizard, removeReportControlAction, selectReportControlWizard, reportControlCopyToIedSelector, } from '../../../src/wizards/reportcontrol.js';
...
describe('Wizards for SCL ReportControl element', () => {
    ...
    describe('define an edit wizard that', () => {
        describe('for complete ReportControl element', () => {
            beforeEach(async () => {
                const wizard = editReportControlWizard(doc.querySelector('ReportControl'));
            ...
            it('edits bufTime attribute only for valid inputs', async () => {
                await fc.assert(fc.asyncProperty(integer({ min: 0 }).map(num => `${num}`), async (bufTime) => {
            ...
            it('rejects bufTime attribute starting with not being a unsigned int', async () => {
                await fc.assert(fc.asyncProperty(regexString(inverseRegExp.uint, 1), async (bufTime) => {
                    inputs[6].value = bufTime;
                    await inputs[6].requestUpdate();
            ...
            it('edits intgPd attribute only for valid inputs', async () => {
                ...
                await fc.assert(fc.asyncProperty(integer({ min: 0 }).map(num => `${num}`), async (intgPd) => {
                    input.value = intgPd;
                    await input.requestUpdate();
            ...
            it('rejects intgPd attribute starting with not being a unsigned int', async () => {
                ...
                input.nullSwitch?.click();
                await input.requestUpdate();
                await fc.assert(fc.asyncProperty(regexString(inverseRegExp.uint, 1), async (intgPd) => {
                    input.value = intgPd;
                    await input.requestUpdate();
            ...
            it('update a ReportControl element when only name attribute changed', async () => {
                ...
                const updateAction = action.actions[0];
            ...
            it('update a ReportControl element when rptID attribute changed', async () => {
                ...
                expect(updateAction.new.element).to.have.attribute('rptID', 'myNewType/ID');
            });
            ...
            describe('with missing RptEnabled child', () => {
                beforeEach(async () => {
                    const wizard = editReportControlWizard(doc.querySelector('ReportControl[name="ReportCb2"]'));
                ...
                it('creates a RptEnable element when max Client activated', async () => {
                    ...
                    const action = actionEvent.args[0][0].detail.action;
                    expect(action).to.not.satisfy(isSimple);
                    expect(action.actions[0]).to.satisfy(isCreate);
                    const updateAction = action.actions[0];
                    expect(updateAction.new.element.tagName).to.equal('RptEnabled');
                    expect(updateAction.new.element).to.have.attribute('max', '6');
                });
            ...
            describe('contains a remove button that', () => {
                const ln01gse = new DOMParser().parseFromString(`<LN0 lnClass="LLN0" lnType="myType">
                ...
                const ln02smv = new DOMParser().parseFromString(`<LN0 lnClass="LLN0" lnType="myType">
                <DataSet name="myDataSet"/>
                <ReportControl name="myName" datSet="myDataSet"/>
                <SampledValueControl name="myName2" datSet="myDataSet"/>
            </LN0>`, 'application/xml').documentElement;
                const missingparent = (new DOMParser().parseFromString(`<ReportControl name="myName" datSet="myDataSet"/>`, 'application/xml').documentElement);
                it('removes ReportControl and its referenced DataSet if no other ReportControl are assigned', () => {
                    ...
                    const actions = (removeReportControlAction(reportControl).actions);
        ...
        describe('for ReportControl with missing child elements and referenced DataSet', () => {
            beforeEach(async () => {
                const ln0 = new DOMParser().parseFromString('<LN0 lnClass="LNN0"><ReportControl name="testName"/><LN0>', 'application/xml');
    ...
    describe('define a select wizard that', () => {
        describe('with existing ReportControl element', () => {
            ...
            it('looks like the latest snapshot', async () => await expect(element.wizardUI.dialog).dom.to.equalSnapshot()).timeout(5000);
        });
    ...
    describe('define an create wizard that', () => {
        ...
        it('has four pages', () => expect(element.wizardUI.dialogs.length).to.equal(4));
        ...
        it('referenced DataSet element not having any FCDA per default', async () => {
            await primaryAction.click();
            const createAction = (actionEvent.args[0][0].detail.action.actions[1]);
        ...
        it('complex action adding RptEnabled element as child element with nulled max Client', async () => {
            const max = (element.wizardUI.dialogs[0].querySelector('wizard-textfield[label="max Clients"]'));
    ...
    describe('define copy to other IED selector', () => {
        ...
        it('allows to copy to multiple IED at once', () => expect(iedsPicker.multi).to.be.true);
        describe('with a sink IED not meeting partially the data references', () => {
            ...
            it('disabled the list item', () => expect(listItem.disabled).to.not.be.true);
...
//# sourceMappingURL=reportcontrol.test.js.map