...
import { isSimple, } from '../../../src/foundation.js';
import { editIEDWizard, removeIEDAndReferences, removeIEDWizard, } from '../../../src/wizards/ied.js';
import { expectDeleteAction, expectReplaceAction, expectUpdateAction, expectWizardNoUpdateAction, fetchDoc, setWizardTextFieldValue, } from './test-support.js';
...
describe('Wizards for SCL element IED', () => {
    ...
    describe('edit IED', () => {
        ...
        it('contains a wizard-textfield with a non-empty "type" value', async () => {
            expect(inputs.find(textField => textField.label == 'type')
                ?.value).to.be.equal(ied.getAttribute('type') || '-');
        });
        it('contains a wizard-textfield with a non-empty "manufacturer" value', async () => {
            expect(inputs.find(textField => textField.label == 'manufacturer')?.value).to.be.equal(ied.getAttribute('manufacturer') || '-');
        });
        it('contains a wizard-textfield with a non-empty "configVersion" value', async () => {
            expect(inputs.find(textField => textField.label == 'configVersion')?.value).to.be.equal(ied.getAttribute('configVersion') || '-');
        });
        it('contains a wizard-textfield with a non-empty "originalSclVersion" value', async () => {
            expect(inputs.find(textField => textField.label == 'originalSclVersion')?.value).to.contain(ied.getAttribute('originalSclVersion') || '-');
        });
        it('contains a wizard-textfield with an empty "engRight" value', async () => {
            expect(inputs.find(textField => textField.label == 'engRight')?.value).to.be.equal(ied.getAttribute('engRight') || '-');
        });
        it('contains a wizard-textfield with a non-empty "owner" value', async () => {
            expect(inputs.find(textField => textField.label == 'owner')?.value).to.be.equal(ied.getAttribute('owner') || '-');
        });
        it('update name should be updated in document', async function () {
            await setWizardTextFieldValue(inputs[0], 'OtherIED3');
            const complexAction = updateNamingAttributeWithReferencesAction(ied, 'ied.action.updateied')(inputs, element.wizardUI);
        ...
        it('update name should be unique in document', async function () {
            await setWizardTextFieldValue(inputs[0], 'IED2');
        ...
        it('when no fields changed there will be no update action', async function () {
            expectWizardNoUpdateAction(updateNamingAttributeWithReferencesAction(ied, 'ied.action.updateied'), element.wizardUI, inputs);
        });
    ...
    describe('remove IED', () => {
        ...
        it('remove IED should return expected actions', async function () {
            const complexAction = removeIEDAndReferences(ied)(inputs, element.wizardUI);
            expect(complexAction.length).to.equal(1);
            expect(complexAction[0]).to.not.satisfy(isSimple);
            const simpleActions = complexAction[0].actions;
