...
import { getValAction, wizardContent, } from '../../../src/wizards/abstractda.js';
import { regExp, regexString } from '../../foundation.js';
describe('abstractda wizards', () => {
    describe('getValAction', () => {
        const abstractda = new DOMParser().parseFromString(`<DA name="ctlModel" bType="Enum" type=""></DA>`, 'application/xml').documentElement;
        const oldVal = new DOMParser().parseFromString(`<Val>oldVal</Val>`, 'application/xml').documentElement;
    ...
    describe('renderWizard', () => {
        ...
        beforeEach(async () => {
            ...
            enumTypes = Array.from(data.querySelectorAll('EnumType')).map(enumtype => enumtype.getAttribute('id'));
            daTypes = Array.from(data.querySelectorAll('DAType')).map(enumtype => enumtype.getAttribute('id'));
            const wizard = [
                {
                    title: 'title',
                    content: wizardContent('', null, 'Enum', types, 'Dummy_ctlModel', null, null, null, 'status-only', data),
                },
            ];
        ...
        it('edits name attribute only for valid inputs', async () => {
            await fc.assert(fc.asyncProperty(regexString(regExp.abstractDataAttributeName, 1, 32), async (name) => {
        ...
        it('filters Val selection for the initially loaded Enum', async () => {
            await valSelect.requestUpdate();
            expect(valSelect.items.length).to.equal(data.querySelectorAll(`EnumType[id="${typeSelect.value}"] > EnumVal`)
                .length);
        });
...
//# sourceMappingURL=abstractda.test.js.map