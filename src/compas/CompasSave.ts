...
export default class CompasSaveElement extends CompasExistsIn(LitElement) {
  ...
  getNameField(): TextFieldBase {
    return <TextFieldBase>(
      this.shadowRoot!.querySelector('mwc-textfield[id="name"]')
    );
  }

  getSclTypeRadioGroup(): CompasSclTypeRadiogroup {
    return <CompasSclTypeRadiogroup>(
      this.shadowRoot!.querySelector('compas-scltype-radiogroup')
    );
  }
  ...
  valid(): boolean {
    if (!this.existInCompas) {
      return (
        this.getNameField().checkValidity() &&
