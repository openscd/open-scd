 ...
 */
@customElement('later-binding-list')
export class LaterBindingList extends LitElement {
  ...
  private getFcdaElements(ctrlElement: Element): Element[] {
    const lnElement = ctrlElement.parentElement;
    if (lnElement) {
      return Array.from(
        lnElement.querySelectorAll(
          `:scope > DataSet[name=${ctrlElement.getAttribute('datSet')}] > FCDA`
        )
      ).sort((a, b) => compareNames(`${identity(a)}`, `${identity(b)}`));
    }
    return [];
  ...
  private openEditWizard(ctrlElement: Element): void {
    const wizard = wizards[this.controlTag].edit(ctrlElement);
  ...
  private onFcdaSelect(ctrlElement: Element, fcdaElement: Element) {
    this.resetSelection();

    this.selectedControlElement = ctrlElement;
  ...
  renderFCDA(ctrlElement: Element, fcdaElement: Element): TemplateResult {
    return html`<mwc-list-item
      graphic="large"
      twoline
      class="subitem"
      @click=${() => this.onFcdaSelect(ctrlElement, fcdaElement)}
      value="${identity(ctrlElement)} ${identity(fcdaElement)}"
    >
  ...
  render(): TemplateResult {
    ...
    return html` <section tabindex="0">
      ${controlElements.length > 0
        ? html`<h1>
              ${translate(
                `subscription.laterBinding.${this.controlTag}.ctrlList.title`
              )}
            </h1>
            <filtered-list>
              ${controlElements.map(ctrlElement => {
                const fcdaElements = this.getFcdaElements(ctrlElement);
                return html`
                  <mwc-list-item
                    noninteractive
                    graphic="icon"
                    twoline
                    hasMeta
                    value="${identity(ctrlElement)} ${fcdaElements
                  ...
                  >
                    <mwc-icon-button
                      ...
                      icon="edit"
                      class="interactive"
                      @click=${() => this.openEditWizard(ctrlElement)}
                    ></mwc-icon-button>
                    <span
                      >${getNameAttribute(ctrlElement)}
                      ${getDescriptionAttribute(ctrlElement)
                        ? html`${getDescriptionAttribute(ctrlElement)}`
                        : nothing}</span
                    >
                    <span slot="secondary">${identity(ctrlElement)}</span>
                    <mwc-icon slot="graphic">${smvIcon}</mwc-icon>
                  </mwc-list-item>
                  <li divider role="separator"></li>
                  ${fcdaElements.map(fcdaElement =>
                    this.renderFCDA(ctrlElement, fcdaElement)
                  )}
                `;
              })}
            </filtered-list>`
        : html`<h1>
            ${translate(
              `subscription.laterBinding.${this.controlTag}.ctrlList.noCtrlFound`
            )}
          </h1>`}
    </section>`;
  }
