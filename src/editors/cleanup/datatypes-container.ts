...
// import { editDATypeWizard, getGSE } from '../../wizards/gsecontrol.js';
...
export class CleanupControlBlocks extends LitElement {
  ...
  private renderFilterIconButton(
    ...
    return html`<mwc-icon-button-toggle
      ...
      label="filter"
      ?on=${initialState}
      class="t${controlType}Filter filter"
      @click="${(e: MouseEvent) => {
        e.stopPropagation();
      ...
      <span>
        <mwc-icon-button
          ...
          @click=${(e: MouseEvent) => {
            ...
            //     // newSubWizardEvent(editDATypeWizard(controlBlock))
            //   );
            ...
            // } else if (controlBlock.tagName === 'SampledValueControl') {
            //   e.target?.dispatchEvent(
            //     newSubWizardEvent(editEnumTypeWizard(controlBlock))
            //   );
  ...
  private renderUnreferencedControls() {
    ...
    return html`
      <div>
        ...
        ${this.renderFilterIconButton('DOType', false)}
