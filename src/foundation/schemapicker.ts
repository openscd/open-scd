import { getSchema } from '../foundation.js';
import { SchemaVersionNumber } from './schemas.js';
...
export function getAttributeDefinition(
  ...
  const schemaString = getSchema(versionNumber);
  const schema = <XMLDocument>(
    new DOMParser().parseFromString(schemaString, 'application/xml')
  );
