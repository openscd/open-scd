...
export function isEmptyObject(target, dealedAsEmpty = [null, void 0, ""]) {
  return (target === null ? [false] : Object.keys(target).flatMap((key) => {
...
export function editServicesWizard(services) {
  ...
  ].filter((page) => page !== null).map((page) => page);
}
