export function getConnectedAP(element) {
  return element.ownerDocument.querySelector(`:root > Communication > SubNetwork > ConnectedAP[iedName="${element.closest("IED")?.getAttribute("name")}"][apName="${element.closest("AccessPoint")?.getAttribute("name")}"]`);
}
...
export function uniqueMacAddress(doc, serviceType) {
  ...
  const givenMacs = Array.from(doc.querySelectorAll("Address > P")).filter((pElement) => pElement.getAttribute("type") === "MAC-Address").map((mac2) => mac2.innerHTML.trim());
  let mac = startMac;
  while (mac !== maxMac) {
...
function incrementAppId(oldAppId) {
  return (parseInt(oldAppId, 16) + 1).toString(16).toUpperCase().padStart(4, "0");
}
export function uniqueAppId(doc) {
  const maxAppId = "FFFF";
  const startAppId = "0001";
  const givenAppIds = Array.from(doc.querySelectorAll("Address > P")).filter((pElement) => pElement.getAttribute("type") === "APPID").map((mac) => mac.innerHTML.trim());
