...
export function getSMVDataChildren(parent) {
  if (parent.tagName === "Server")
    return Array.from(parent.children).filter((child) => child.tagName === "LDevice" && child.querySelector('LN[lnClass="TCTR"],LN[lnClass="TVTR"]'));
  if (parent.tagName === "LDevice")
    return Array.from(parent.children).filter((child) => child.tagName === "LN" && (child.getAttribute("lnClass") === "TCTR" || child.getAttribute("lnClass") === "TVTR"));
  const id = parent.tagName === "LN" || parent.tagName === "LN0" ? parent.getAttribute("lnType") : parent.getAttribute("type");
