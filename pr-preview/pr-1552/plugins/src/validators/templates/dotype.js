...
async function getSpecificDataObject(lnClass, doName) {
  ...
  return lnodeclasses.flatMap((lnodeclass) => Array.from(lnodeclass.querySelectorAll(`DataObject`))).find((dataObject) => dataObject.getAttribute("name") === doName) ?? null;
}
async function getNsdReference(element) {
  const id = element.getAttribute("id");
  if (!id)
    return null;
  const doorsdo = element.closest("DataTypeTemplates")?.querySelector(`LNodeType > DO[type="${id}"], LNodeType > SDO[type="${id}"]`);
