import { getNameAttribute, getPathNameAttribute, } from '../../../../openscd/src/foundation.js';
...
export function getConnectedTerminals(element) {
    ...
    return Array.from(substationElement.getElementsByTagName('Terminal')).filter(terminal => terminal.getAttribute('connectivityNode') === path &&
...
export function calculateConnectivityNodeCoordinates(cNodeElement) {
    ...
    let totalX = 0;
    let totalY = 0;
    Array.from(substationElement.querySelectorAll('ConductingEquipment, PowerTransformer'))
        .filter(equipment => equipment.querySelector(`Terminal[connectivityNode="${pathName}"]`) !=
        null)
        .forEach(equipment => {
        nrOfConnections++;
