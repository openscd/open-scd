...
async function getServiceChildren(daName) {
  ...
  return Array.from(nsd81.querySelectorAll(`ServiceConstructedAttributes > ServiceConstructedAttribute[name="${daName}"] >  SubDataAttribute[presCond="M"]`));
}
async function getMandatoryChildren(datype) {
  const id = datype.getAttribute("id");
  if (!id)
    return [];
  const dataAttribute = datype.closest("DataTypeTemplates")?.querySelector(`DOType > DA[type="${id}"]`);
