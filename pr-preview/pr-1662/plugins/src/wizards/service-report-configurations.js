...
function createReportConfigurationsWizard(parent) {
  const content = {
    reportSettings: {
      ...
      optFields: parent.querySelector("ReportSettings")?.getAttribute("optFields") ?? null,
    ...
    confReportControl: {
      ...
      bufMode: parent.querySelector("ConfReportControl")?.getAttribute("bufMode") ?? null,
      maxBuf: parent.querySelector("ConfReportControl")?.getAttribute("maxBuf") ?? null,
      bufConf: parent.querySelector("ConfReportControl")?.getAttribute("bufConf") ?? null
    },
    clientServices: {
      maxReports: parent.querySelector("ClientServices")?.getAttribute("maxReports") ?? null,
      bufReport: parent.querySelector("ClientServices")?.getAttribute("bufReport") ?? null,
      unbufReport: parent.querySelector("ClientServices")?.getAttribute("unbufReport") ?? null
    },
    dynDataSet: {
      max: parent.querySelector("DynDataSet")?.getAttribute("max") ?? null,
      maxAttributes: parent.querySelector("DynDataSet")?.getAttribute("maxAttributes") ?? null
    }
  };
