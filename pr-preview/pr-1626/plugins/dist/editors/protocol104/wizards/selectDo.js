...
import { compareNames, getNameAttribute, identity, newSubWizardEvent, find, } from '../../../../../openscd/src/foundation.js';
...
import { getCdcValueFromDOElement, getDoElements, } from '../foundation/foundation.js';
/**
...
function filterAvailableElements(child) {
    ...
    return (lnElements.filter(lnElement => 
    // Check if there are available DO Elements that aren't initiated and supported by 104 protocol
    getDoElements(lnElement).filter(doElement => filterAvailableDOElements(lnElement, doElement)).length > 0).length > 0);
}
/**
...
export function getDataChildren(parent) {
    ...
    if (['LN0', 'LN'].includes(parent.tagName)) {
        ...
        children = Array.from(parent.ownerDocument.querySelectorAll(`:root > DataTypeTemplates > LNodeType[id="${lnType}"] > DO`))
        ...
        children = Array.from(parent.children)
            .filter(child => ['IED', 'AccessPoint', 'LN0', 'LN'].includes(child.tagName))
            .filter(child => filterAvailableElements(child))
            .sort((a, b) => compareNames(`${identity(a)}`, `${identity(b)}`));
    }
    return children;
...
//# sourceMappingURL=selectDo.js.map