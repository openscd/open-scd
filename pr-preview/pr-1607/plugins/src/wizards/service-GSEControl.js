...
function createGSEControlWizard(parent) {
  const content = {
    gseSettings: {
      ...
      kdaParticipant: parent.querySelector("GSESettings")?.getAttribute("kdaParticipant") ?? null,
      signature: parent.querySelector("GSESettings > McSecurity")?.getAttribute("signature") ?? null,
      encryption: parent.querySelector("GSESettings > McSecurity")?.getAttribute("encryption") ?? null
    },
    goose: {
    ...
    clientServices: {
      maxGOOSE: parent.querySelector("ClientServices")?.getAttribute("maxGOOSE") ?? null,
